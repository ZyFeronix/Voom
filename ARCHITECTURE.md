...
- **Control de Sesiones:** JSON Web Tokens (JWT) gestionados a través de Cookies seguras `httpOnly` y middleware de autenticación (Hooks de SvelteKit).

### 1.3 Infraestructura de Datos
- **Motor de Base de Datos:** SQLite 3 transaccional.
- **Driver de Conexión:** `better-sqlite3` (Síncrono, hiper-rápido).
...
La base de datos (`data/vsocial.db`) está diseñada para normalización de datos relacionales puros.

1. **Tabla `users`:**
   - `id`, `username`, `email`, `password_hash`.
   - Perfilamiento: `display_name`, `bio`, `avatar_url`, `banner_url`.
