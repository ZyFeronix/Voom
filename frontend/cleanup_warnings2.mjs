...
content = content.replace(/<label class="flex justify-between text-muted text-xs italic">/g, '<span class="flex justify-between text-muted text-xs italic">');
content = content.replace(/<\/label>\s*<\/div>\s*<\/div>\s*<\/div>\s*<!-- Background Upload/g, '</span>\n            </div>\n          </div>\n        </div>\n\n        <!-- Background Upload');
