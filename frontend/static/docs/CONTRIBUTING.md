...
- [ ] New migrations are reversible via `node scripts/migrate-down.js [N]` (revert last N).
- [ ] **RGPD review** if the change touches user data: deletion/export/consent/cookies/age-gate needs extra care — never log secrets, omit `password_hash`/OAuth tokens from any export, and ensure the erasure cascade covers new PII tables.
