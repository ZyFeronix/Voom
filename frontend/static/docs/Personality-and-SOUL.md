...
1. **Seguridad** — Sin excepciones. Sin compromisos. JWT en `localStorage` + cookie client-side `Secure; SameSite=Strict` (no httpOnly: el cliente lee el token en `lib/api.js`), prepared statements, CSRF por validación de Origin, CSP estricto.
...
- **JWT:** Token único (caducidad `JWT_EXPIRES_IN`, default 365d) guardado en `localStorage` (`vsocial_token`) y espejado en una cookie client-side `Secure; SameSite=Strict` (no httpOnly — `lib/api.js` lee el token para enviarlo como Bearer). Hash del token (SHA-256) en `user_sessions` con IP + user-agent. *Directriz futura: mover a cookies httpOnly + access/refresh rotativos es una mejora pendiente.*
...
- **Consentimiento y edad:** El registro exige aceptar Términos + Privacidad + confirmar ≥13 años, con sellado de tiempo (`terms_accepted_at`/`privacy_accepted_at`). Age gate validado en servidor.
- **Cookies:** Banner de consentimiento (`CookieBanner.svelte`); sin analítica/publicidad de terceros. Política en `/cookies`.
- **Derecho al olvido:** IMPLEMENTADO. `POST /api/users/delete-account` (confirmación con contraseña) → soft-delete (`deleted_at`, `is_active=0`) + cierre de sesiones; reactivación al login dentro de 30 días; pasado ese plazo, un cron diario en `hooks.server.js` hard-deletea en cascada y limpia los ficheros huérfanos.
- **Portabilidad:** `GET /api/users/export` devuelve JSON con todos los datos del usuario (omitiendo secretos). Páginas legales en `/privacy`, `/terms`, `/cookies`.
- **Auditoría:** Logs inmutables de todas las acciones de administración y acceso a datos sensibles. *(Nota: la purga/retención por tiempo de `activity_logs` queda pendiente.)*
...
| **Autenticación** | JWT en `localStorage` + cookie client-side `Secure; SameSite=Strict` + middleware en hooks de SvelteKit. |
