# Guía de empaquetado de releases (.zip) — Voom!
...
> Última release generada con este proceso: **v0.6.0-beta.1** (`voom-v0.6.0-beta.1.zip`, ~6.2 MB).
...
- `frontend/static/` (favicon, logo, manifest, robots, LICENSE.txt, media, sounds, emoticons, **docs/** con index.html y portal estático).
- Configs: `package.json`, `package-lock.json`, `.npmrc`, `.prettierrc`,
  `.prettierignore`, `.gitignore`, `eslint.config.js`, `jsconfig.json`, `svelte.config.js`,
  `vite.config.js`, `vitest.config.js`, `README.md`, `server.js`.
...
### Archivos de agentes IA / herramientas / notas internas (uso interno)
`PACKAGING.md`, `Personality & SOUL.md`, `docs/` (raíz),
`.agents/`, `.ai-codex/`, `.claude/`, `.codegraph/`, `.opencode/`, `.qoder/`, `opencode.jsonc`.
...
> Nota sobre `npm run lint`: los dos bugs anteriores están corregidos y el lint
> del proyecto **sale verde** (`prettier --check . && eslint .` sin errores,
> verificado ago 2026). Si al armar un release aparece un fallo de formato,
> ejecutar `npm run format` antes de comprimir; no debe ignorarse.
...
Copy-Item -Recurse -Force (Join-Path $root 'media')      (Join-Path $target 'media')
Copy-Item -Recurse -Force (Join-Path $root '.husky')     (Join-Path $target '.husky')
Copy-Item -Recurse -Force (Join-Path $root 'tests')      (Join-Path $target 'tests')
...
3. **`npm run lint`** — confirma que **no** aparezca el crash
   ...
   `.prettierrc`). Debe salir **totalmente verde**: si hay fallos de formato,
   ejecutar `npm run format` y re-empaquetar.
...
- Desde la release **v0.5** el número del `.zip` **sí** está sincronizado con
  `frontend/package.json` y con `CHANGELOG.md`. (Históricamente 0.0.2 no estaba
  alineado; v0.5 corrigió el desfase.)
- La release actual es **v0.6.0-beta.1** (`voom-v0.6.0-beta.1.zip`, `frontend/package.json`
  → `"version": "0.6.0-beta.1"`, `CHANGELOG.md` → `## [0.6.0-beta.1]`). Con sufijos
  de pre-release (`-beta.X`, `-rc.X`) el nombre del `.zip` usa la versión completa
  (`voom-v0.6.0-beta.1.zip`), no la forma corta `vX.Y`.
- Al armar una nueva release, mantener la coherencia actualizando en el mismo cambio:
