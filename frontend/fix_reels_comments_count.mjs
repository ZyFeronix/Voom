...
async function main() {
  ...
  for (const reel of reelsResult) {
    const actualResult = await db.prepare('SELECT COUNT(*) as c FROM reel_comments WHERE reel_id = ?').get(reel.id);
    const actualCount = actualResult.c;
    
    if (actualCount !== reel.comment_count) {
      console.log(`Reel ${reel.id}: DB says ${reel.comment_count}, actual is ${actualCount}. Fixing...`);
