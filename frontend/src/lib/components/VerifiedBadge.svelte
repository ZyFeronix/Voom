...
{#if badgeType !== 'none'}
  ...
  <div bind:this={badgeEl} class="custom-badge-wrapper {interactive ? 'is-interactive' : ''}" role={interactive ? 'button' : 'presentation'} tabindex={interactive ? '0' : '-1'} onclick={togglePopup} style="--badge-color: {badgeData.color}; --badge-size: {size};">
    ...
    {#if showPopup}
      ...
      >
        ...
        <div class="popup-body">
          <span class="material-icons-round popup-body-icon" style="color: {badgeData.color}">{badgeData.icon}</span>
          <p class="popup-desc">
...
<style>
  .custom-badge-wrapper {
    ...
    padding: 2px 4px;
    box-shadow: 0 2px 6px color-mix(in srgb, var(--badge-color) 15%, transparent), inset 0 1px 0 rgba(255,255,255,0.05);
  }
  ...
  .badge-popup {
    ...
    width: 250px;
    background: radial-gradient(circle at top right, rgba(255, 255, 255, 0.03), transparent 60%), rgba(10, 14, 20, 0.85);
    backdrop-filter: blur(25px);
    -webkit-backdrop-filter: blur(25px);
    border: 1px solid rgba(255, 255, 255, 0.08);
    box-shadow: 0 15px 45px rgba(0, 0, 0, 0.6), inset 0 1px 0 rgba(255, 255, 255, 0.1), 0 0 25px var(--badge-color, rgba(0,0,0,0));
    border-radius: 12px;
    padding: 14px;
    z-index: 1000;
    cursor: default;
