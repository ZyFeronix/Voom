				...
				<div class="flex items-center gap-3">
					<div class="like-btn-wrapper">
						<button class="like-btn flex items-center gap-1.5" onclick={toggleLike}>
							<span
								class="material-icons-round like-icon {likeAnim ? 'heart-pop' : ''}"
								class:liked-icon={liked}
								style="font-size:15px">{liked ? 'favorite' : 'favorite_border'}</span
							>
							<span class="like-count font-medium" class:liked-count={liked}
				...
				<div class="reply-input-wrapper mt-3 relative" transition:fade={{ duration: 150 }}>
					...
					<div class="reply-cancel-row">
						<button
							...
							class="reply-cancel-btn">Cancelar</button
						>
					</div>
				</div>
			{/if}
...
<style>
	...
	@keyframes heartExplosion {
		...
		20% {
			...
			filter: drop-shadow(0 0 12px var(--aero-rose, #ec4899));
			color: var(--aero-rose, #ec4899);
		}
		40% {
	...
	/* Móvil: blur más ligero en las superficies hardcoded de comentarios (4 capas por
	   comentario, anidadas). Solo ≤768px; desktop idéntico. */
