<script>
	...
	let { streak = 0, canCheckin = false, nextCheckin = null, disabled = false, oncheckin } = $props();
...
{#if canCheckin && !disabled}
	...
	>
		...
		>
			<!-- Neo-Aero Glass Base -->
			<div class="absolute inset-0 bg-white/10 backdrop-blur-xl border border-white/20 z-0" style="box-shadow: inset 0 1px 1px rgba(255,255,255,0.3), inset 0 -1px 2px rgba(0,0,0,0.1);"></div>

			<!-- Barra de progreso líquida Neo-Aero -->
			<div class="absolute top-0 left-0 h-full fill-bar pointer-events-none flex items-center justify-end shadow-[0_0_20px_var(--neon-primary)] z-0" style="background: linear-gradient(90deg, var(--aero-blue), var(--aero-sky));">
			...
			<div class="absolute inset-0 bg-white/60 opacity-0 transition-opacity duration-300 pointer-events-none mix-blend-overlay z-0" class:opacity-100={progress >= 100}></div>
			
			<!-- Contenido Central -->
			<div class="relative z-10 flex items-center gap-2 text-white font-bold tracking-wide pointer-events-none flex-1 justify-center" style="text-shadow: 0 1px 4px rgba(0,0,0,0.4);">
				<span class="material-icons-round" style="font-size: 22px; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.3));" class:animate-bounce={progress >= 100}>
			...
			<div class="relative z-10 flex-shrink-0 ml-2">
				{#if streak > 0}
					<div class="flex items-center gap-1.5 text-white bg-black/40 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold border border-white/10 shadow-[0_2px_8px_rgba(0,0,0,0.4)]">
						<span style="filter: drop-shadow(0 0 6px rgba(255,165,0,0.8));">🔥</span> {streak}
					</div>
				{/if}
			</div>
		</button>
