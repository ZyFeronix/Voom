<script>
	let { title = '', color = 'blue', size = 'md' } = $props();
	...
	let sizeClass = $derived(size === 'sm' ? 'text-[10px] px-2 py-0.5' : 'text-xs px-2.5 py-1');
</script>

<div class="inline-flex items-center justify-center font-bold uppercase tracking-wider rounded-full border backdrop-blur-sm {themeClass} {sizeClass}">
	{title}
</div>
