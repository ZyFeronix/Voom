<script>
		...
		const typeStr =
			log.entity_type === 'post'
				...
				: log.entity_type === 'comment'
					? 'un comentario'
					: log.entity_type === 'reel'
						? 'un reel'
						: log.entity_type === 'video'
							? 'un video'
							: 'un elemento';

		switch (log.action_type) {
			case 'view':
				return `Visualizaste ${typeStr}`;
			case 'like':
				return `Te gustó ${typeStr}`;
			case 'unlike':
				return `Quitaste tu me gusta de ${typeStr}`;
			case 'comment':
				return `Comentaste en ${typeStr}`;
			case 'create':
				return `Creaste ${typeStr}`;
			case 'update':
				return `Editaste ${typeStr}`;
			case 'delete':
				return `Eliminaste ${typeStr}`;
			default:
				return `Interactuaste con ${typeStr}`;
		}
	}
	...
	function getLink(log) {
		...
		if (log.entity_type === 'comment') {
			if (log.metadataObj?.post_id)
				return `/posts/${log.metadataObj.post_id}#comment-${log.entity_id}`;
			if (log.metadataObj?.reel_id)
				return `/reels?id=${log.metadataObj.reel_id}#comment-${log.entity_id}`;
				...
				{:else}
					<div class="flex flex-col gap-4">
						{#each logs as log (log.id)}
							<a
								href={getLink(log)}
								class="group relative flex items-start gap-4 p-4 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 border border-transparent transition-colors duration-200 {getLink(
									log
								) === '#'
									? 'cursor-default'
									: 'cursor-pointer'}"
							>
								...
								<div
									class="absolute left-9 top-14 bottom-[-1rem] w-[2px] bg-black/10 dark:bg-white/10 group-last:hidden"
								></div>
								...
								<div class="flex-1 min-w-0 pt-1">
									...
									{#if log.entityPreview}
										<div
											class="mt-2 p-3 rounded-lg bg-black/5 dark:bg-black/20 border border-black/10 dark:border-white/5 text-sm text-muted line-clamp-2"
										>
											{#if ['delete', 'update'].includes(log.action_type)}
												<span
													class="text-xs font-mono text-yellow-600 dark:text-yellow-400/70 mb-1 block"
													>Contenido original antes del cambio:</span
												>
											{/if}
											{log.entityPreview}
										</div>
