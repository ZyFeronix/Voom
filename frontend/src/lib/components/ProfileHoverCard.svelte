...
<div bind:this={triggerEl} class="hover-trigger-wrap" onmouseenter={handleMouseEnter} onmouseleave={handleMouseLeave} role="presentation">
  {@render children()}
      ...
      {#if fullUser}
        <div class="hc-header" style="position: relative; z-index: 1;">
          {#if fullUser.cover_url}
            <img src={fullUser.cover_url} alt="Cover" class="hc-cover-img" />
          {:else}
        ...
        <div class="hc-body" style="position: relative; z-index: 1;">
          <div class="hc-top-row">
            {#if fullUser.avatar_url}
              <img src={fullUser.avatar_url} alt={username} class="hc-avatar" />
            {:else}
            ...
            {#if fullUser.id !== authStore.user?.id && authStore.isAuthenticated}
              <button class="btn-aero-primary hc-follow-btn {fullUser.is_following ? 'following' : ''}" onclick={toggleFollow}>
          ...
          <div class="hc-user-info">
            <div class="hc-name-row">
              ...
              <VerifiedBadge role={fullUser.role} isVerified={fullUser.is_verified == 1} size="16px" />
              {#if fullUser.level}
                <LevelBadge level={fullUser.level} size="sm" showText={false} />
              {/if}
              {#if fullUser.title_text}
                <UserTitleBadge title={fullUser.title_text} color={fullUser.title_color} size="sm" />
              {/if}
            </div>
          ...
          <div class="hc-level">
            <span class="material-icons-round text-amber-400" style="font-size: 18px;">military_tech</span>
