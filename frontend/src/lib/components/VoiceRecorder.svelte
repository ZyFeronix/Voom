...
<div class="voice-recorder-container variant-{variant}">
			...
			<div style="flex: 1; min-width: 0;">
				<div class="custom-select-wrapper">
					<button
						...
						type="button"
					>
						<span class="truncate" style="min-width: 0; flex: 1; text-align: left;"
							>{devices.find((d) => d.deviceId === selectedDeviceId)?.label ||
								'Seleccionar micrófono...'}</span
						>
						<span
							class="material-icons-round"
							style="font-size: 16px; margin-left: 4px; transition: transform 0.2s; flex-shrink: 0;"
							class:rotate={showMicDropdown}>expand_more</span
						>
					</button>
							...
							>
								{#each devices as device}
									<button
										...
										type="button"
									>
										<span class="material-icons-round check-icon" style="flex-shrink: 0;"
											>{device.deviceId === selectedDeviceId ? 'check' : ''}</span
										>
										<span class="truncate" style="min-width: 0; flex: 1; text-align: left;"
											>{device.label || 'Micrófono desconocido'}</span
										>
									</button>
								{/each}
							</div>
	...
	{:else}
		<div class="recording-indicator">
			<div
				style="flex: 0 0 32px; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center;"
			>
