<script>
  ...
  function handleMessageReaction(e) {
    const { message_id, emoji, action, user_reacted } = e.detail || {};
  ...
  async function loadConversations(isBackground = false) {
    ...
    try {
      ...
      if (activeConvId) {
        newConvs = newConvs.map(c => Number(c.id) === Number(activeConvId) ? { ...c, unread_count: 0 } : c);
      }
  ...
  async function openChat(id) {
    ...
    conversations = conversations.map(c => Number(c.id) === Number(id) ? { ...c, unread_count: 0 } : c);
  ...
  async function sendMessage(e) {
    ...
    try {
      ...
      const realMsg = res.message || { id: res.message_id, body: text, sender_id: authStore.user?.id };

      if (messages.some(m => Number(m.id) === Number(realMsg.id))) {
        messages = messages.filter(m => m.id !== tempId);
      } else {
        messages = messages.map(m => m.id === tempId ? { ...m, ...realMsg, pending: false } : m);
      }
  ...
  onDestroy(() => { processedMessageIds.clear(); });
</script>
...
<div class="glass-panel quick-chat-widget">
  {#if !activeConvId}
    ...
    <div class="qc-list-view" in:fade={{ duration: 180 }}>
      <div class="qc-list-header">
        <h3 class="qc-title">
          <span class="material-icons-round" style="font-size:15px;color:var(--aero-blue)">chat_bubble</span>
          Quick Chat
        </h3>
          ...
          {#each conversations as conv}
            <button class="qc-conv-item" onclick={() => openChat(conv.id)}>
              <div class="qc-conv-avatar-wrap">
                {#if conv.peer_avatar}
                  <img src={conv.peer_avatar} alt="Avatar" class="qc-conv-avatar-img" />
                {:else}
              ...
              <div class="qc-conv-info">
                <div class="qc-conv-top">
                  <span class="qc-conv-name">{conv.name || conv.peer_display_name || conv.peer_username}</span>
                  {#if conv.last_message_at}
                    <span class="qc-conv-time">{new Date(conv.last_message_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                  {/if}
                </div>
  ...
  {:else}
    ...
    <div class="qc-chat-view" in:fade={{ duration: 180 }}>
      ...
      <div class="qc-chat-header">
        ...
        <div class="qc-chat-peer">
          {#if activeConv?.peer_avatar}
            <img src={activeConv.peer_avatar} alt="Avatar" class="qc-peer-avatar" />
          {:else}
          ...
          {/if}
          <div>
            <p class="qc-peer-name">{activeConv?.name || activeConv?.peer_display_name || activeConv?.peer_username || 'Chat'}</p>
      ...
      >
        {#if loadingMsgs}
          ...
          <div class="qc-msg-skeletons">
            <div class="qc-msg-skel peer"><div class="qc-msg-skel-bubble" style="width:72%"></div></div>
            <div class="qc-msg-skel me"><div class="qc-msg-skel-bubble" style="width:55%"></div></div>
            <div class="qc-msg-skel peer"><div class="qc-msg-skel-bubble" style="width:80%"></div></div>
            <div class="qc-msg-skel me"><div class="qc-msg-skel-bubble" style="width:45%"></div></div>
            <div class="qc-msg-skel peer"><div class="qc-msg-skel-bubble" style="width:60%"></div></div>
          </div>
                ...
                >
                  <div class="qc-bubble-wrapper">
                    {#if isMe && !msg.is_deleted}
                      <button class="qc-msg-action-btn" onclick={() => deletingMessageId = msg.id} aria-label="Eliminar mensaje" title="Eliminar mensaje">
                        <span class="material-icons-round" style="font-size: 14px;">delete</span>
                      </button>
                    {/if}
                    <div class="qc-bubble" class:pending={msg.pending} class:deleted={msg.is_deleted} class:qc-emoji-only={!msg.is_deleted && isEmojiOnly(msg.body || msg.content)} use:twemojiAction>
                      {#if msg.is_deleted}
                        <span class="deleted-text"><span class="material-icons-round" style="font-size: 14px; margin-right: 4px; vertical-align: middle;">block</span>Este mensaje fue eliminado</span>
                      {:else}
                        {#if msg.media_type === 'audio' || msg.media_type === 'video' || msg.voice_url}
                          <div class="mb-1">
                            <MediaPlayer type={msg.media_type || 'audio'} src={msg.media_url || msg.voice_url} />
                          </div>
                        {/if}
      ...
      <!-- Input -->
      <div class="qc-input-area" style="display: grid; align-items: center; grid-template-columns: minmax(0, 1fr);">
        {#if showVoiceRecorder}
          <div class="w-full" style="grid-area: 1 / 1;" in:fly={{ y: 15, duration: 300, easing: backOut }} out:fade={{ duration: 150 }}>
             <VoiceRecorder 
                variant="compact"
                onrecorded={(blob) => {
        ...
        {:else}
          <form onsubmit={sendMessage} class="qc-form relative flex items-center gap-2" style="grid-area: 1 / 1;" in:fade={{ duration: 200, delay: 150 }} out:fade={{ duration: 150 }}>
            <button type="button" class="qc-action-btn" onclick={() => showVoiceRecorder = !showVoiceRecorder} aria-label="Nota de voz">
...
<style>
  ...
  .qc-conv-item:hover { background: rgba(255,255,255,0.05); }

  .qc-conv-avatar-wrap { position: relative; flex-shrink: 0; }
  ...
  .qc-conv-info { flex: 1; min-width: 0; }
  ...
  .qc-conv-preview.unread { font-weight: 700; color: var(--text-primary); }
  ...
  .skel-avatar {
    ...
    background-image: linear-gradient(90deg, rgba(255,255,255,0.04) 0%, rgba(255,255,255,0.12) 40%, rgba(255,255,255,0.04) 80%);
    background-size: 200% 100%;
    animation: qc-shimmer 1.4s ease-in-out infinite;
  }

  .skel-text { flex: 1; display: flex; flex-direction: column; gap: 0; }
  ...
  .qc-conv-skeleton:nth-child(2) .skel-line { animation-delay: 0.15s; }
  .qc-conv-skeleton:nth-child(3) .skel-avatar,
  .qc-conv-skeleton:nth-child(3) .skel-line { animation-delay: 0.3s; }
  ...
  .qc-back-btn:hover { background: rgba(255,255,255,0.08); color: var(--text-primary); }
  ...
  .qc-msg-skel.me { justify-content: flex-end; }
  .qc-msg-skel.peer { justify-content: flex-start; }

  .qc-msg-skel-bubble {
    height: 32px;
    border-radius: 14px;
    background: rgba(255,255,255,0.06);
    background-image: linear-gradient(90deg, rgba(255,255,255,0.04) 0%, rgba(255,255,255,0.14) 40%, rgba(255,255,255,0.04) 80%);
    background-size: 200% 100%;
    animation: qc-shimmer 1.4s ease-in-out infinite;
  }

  .qc-msg-skel:nth-child(1) .qc-msg-skel-bubble { animation-delay: 0s; }
  .qc-msg-skel:nth-child(2) .qc-msg-skel-bubble { animation-delay: 0.1s; }
  .qc-msg-skel:nth-child(3) .qc-msg-skel-bubble { animation-delay: 0.2s; }
  .qc-msg-skel:nth-child(4) .qc-msg-skel-bubble { animation-delay: 0.3s; }
  .qc-msg-skel:nth-child(5) .qc-msg-skel-bubble { animation-delay: 0.4s; }
  ...
  .qc-emoji-only {
    ...
    padding: 0 !important;
  }.qc-bubble-row.me { justify-content: flex-end; }
  .qc-bubble-row.peer { justify-content: flex-start; }
  ...
  .qc-bubble-row.me .qc-bubble-wrapper { align-items: flex-end; }
  .qc-bubble-row.peer .qc-bubble-wrapper { align-items: flex-start; }
  ...
  .qc-action-btn:hover { background: rgba(255,255,255,0.1); }
  .qc-action-btn:active { background: rgba(255,255,255,0.15); }
