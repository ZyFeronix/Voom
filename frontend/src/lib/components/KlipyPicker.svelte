<script>
  ...
  function handleSelect(gif) {
    if (onSelect) {
      ...
      const url = gif.file?.md?.gif?.url || gif.file?.sm?.gif?.url || gif.url || (gif.media && gif.media[0] ? gif.media[0].url : gif.id);
      onSelect(url, gif);
    }
  }
...
<div class="klipy-picker glass-card aero-modal" role="presentation" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()}>
  ...
  <div class="gif-grid custom-scrollbar">
    ...
    {:else}
      {#each gifs as gif}
        {@const previewUrl = gif.file?.sm?.gif?.url || gif.file?.xs?.gif?.url || gif.preview_url || gif.url || (gif.media && gif.media[0] ? gif.media[0].url : '')}
        <!-- svelte-ignore a11y_click_events_have_key_events -->
        <div class="gif-btn" role="button" tabindex="0" onclick={() => handleSelect(gif)} title={gif.title || 'GIF'}>
...
<style>
  ...
  .search-input:focus {
    ...
    box-shadow: 0 0 0 3px rgba(27, 133, 243, 0.15), var(--input-shadow-inner);
  }
  .close-btn {
    position: absolute;
    right: 6px;
  ...
  .gif-btn:hover {
    ...
    box-shadow: 0 8px 16px rgba(27, 133, 243, 0.15), 0 0 0 1px var(--aero-blue);
