...
import { encodeToken } from './jwt.js';
...
export async function handleGoogleOAuth(code, origin) {
	const db = getDb();
	const clientId = db.prepare("SELECT value FROM system_settings WHERE key = 'oauth_google_client_id'").get()?.value;
	const clientSecret = db.prepare("SELECT value FROM system_settings WHERE key = 'oauth_google_client_secret'").get()?.value;
	...
	const existing = db.prepare('SELECT user_id FROM oauth_accounts WHERE provider = ? AND provider_uid = ?')
		.get('google', profile.id);

	let userId;
	if (existing) {
		userId = existing.user_id;
		db.prepare('UPDATE oauth_accounts SET access_token = ?, refresh_token = ? WHERE provider = ? AND provider_uid = ?')
		...
		if (emailMatch) {
			...
			const username = profile.email.split('@')[0].replace(/[^a-zA-Z0-9_]/g, '') + '_' + Date.now().toString(36).slice(-4);
			const result = db.prepare(
				'INSERT INTO users (username, email, password_hash, display_name, avatar_url, email_verified, role) VALUES (?, ?, ?, ?, ?, 1, \'user\')'
			).run(username, profile.email, randomPass, profile.name || username, profile.picture || null);
			userId = Number(result.lastInsertRowid);
			db.prepare("INSERT OR IGNORE INTO user_roles (user_id, role) VALUES (?, 'user')").run(userId);
			db.prepare('INSERT OR IGNORE INTO user_settings (user_id) VALUES (?)').run(userId);
		}
		db.prepare('INSERT INTO oauth_accounts (user_id, provider, provider_uid, email, display_name, avatar_url, access_token, refresh_token) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
			.run(userId, 'google', profile.id, profile.email, profile.name, profile.picture, tokens.access_token, tokens.refresh_token || null);
	}
