...
<svelte:head>
	<title>V-SOCIAL | Acceder</title>
</svelte:head>

<div
	class="login-container min-h-screen w-full flex relative overflow-hidden"
...
>
		...
		>
			...
			<div>
				...
				<p class="text-sm text-muted leading-relaxed">
					El universo donde los creadores virtuales conectan, comparten y prosperan bajo un entorno
					cristalino y fluido.
				</p>
			</div>
			...
			<div class="flex flex-col gap-3 w-full text-left">
				{#each [{ icon: 'water_drop', label: 'Estética Frutiger Aero pura y fluida' }, { icon: 'chat', label: 'Mensajería instantánea en tiempo real' }, { icon: 'storefront', label: 'Marketplace de recursos digitales' }] as item}
	...
	>
		<div class="auth-card max-w-md w-full">
			...
			<!-- Form card -->
			<div
				class="w-full max-w-md {mounted ? 'visible' : 'hidden-state'} transition-all duration-500"
			>
				<div class="glass-panel p-8">
					<div class="mb-6">
						<h2 class="text-2xl font-black text-main title-display mb-1">Bienvenido</h2>
					...
					<form onsubmit={handleLogin} class="flex flex-col gap-4">
						...
						<div class="flex flex-col gap-1.5">
							<label for="identifier" class="text-xs font-bold text-muted"
								>Usuario o correo electrónico</label
							>
							<div class="relative" style="position: relative;">
								<span
									class="material-icons-round text-[18px] text-muted"
					...
					<p class="text-center text-[10px] text-muted mt-6">
						...
						<a href="/terms" class="text-link">Términos</a> y
