        ...
        <div class="report-card glass-card">
          ...
          {#if report.status === 'pending'}
            <div class="report-actions">
              <button class="btn-aero-secondary btn-sm" onclick={() => resolveReport(report.id, 'dismissed')}>Ignorar</button>
              <button class="btn-aero-danger btn-sm" onclick={() => resolveReport(report.id, 'resolved')}>Resolver</button>
            </div>
          {/if}
        </div>
      {/each}
...
<style>
  ...
  .report-tag.pending { background: rgba(232, 160, 35, 0.15); color: var(--aero-amber); }
  .report-time { font-size: 0.75rem; color: var(--text-muted); }

  .report-body { display: flex; flex-direction: column; gap: 8px; }
  .entity-type { font-size: 1rem; font-weight: 700; margin: 0; color: var(--text-primary); text-transform: capitalize; }
  .report-reason { font-size: 0.85rem; color: var(--text-secondary); margin: 0; line-height: 1.4; }
  .reporter { font-size: 0.75rem; color: var(--text-muted); margin: 0; }
  ...
  .empty-state { text-align: center; padding: 64px; color: var(--text-muted); border-radius: var(--radius-lg); }
  .empty-state .material-icons-round { font-size: 48px; opacity: 0.5; margin-bottom: 16px; }
