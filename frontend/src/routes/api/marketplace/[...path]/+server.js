...
async function fetchListingMedia(db, listIds) {
	...
	const rows = await db.prepare(`SELECT listing_id, media_url FROM listing_media WHERE listing_id IN (${ph})`).all(...listIds);
	const map = {};
	for (const m of rows) { if (!map[m.listing_id]) map[m.listing_id] = []; map[m.listing_id].push(m.media_url); }
	return map;
}
...
export async function GET({ request, url, params }) {
	...
	if (action === 'search') {
		...
		const mediaMap = await fetchListingMedia(db, listings.map(l => l.id));
		listings.forEach(item => { item.media = mediaMap[item.id] || []; item.image_url = item.media[0] || null; });
	...
	if (action && /^\d+$/.test(action)) {
		...
		listing.media = await db.prepare('SELECT media_url FROM listing_media WHERE listing_id = ?').all(parseInt(action)).map(r => r.media_url);
...
export async function POST({ request, url, params }) {
	...
	if (!action) {
		...
		const result = await db.prepare("INSERT INTO marketplace_listings (user_id, category_id, title, description, price, condition, status) VALUES (?, ?, ?, ?, ?, ?, 'active')")
		...
		for (let i = 0; i < mediaUrls.length; i++) {
			await db.prepare('INSERT INTO listing_media (listing_id, media_url, position) VALUES (?, ?, ?)').run(listingId, mediaUrls[i], i);
		}
	...
	if (action && /^\d+$/.test(action) && parts[1] === 'offers') {
		...
		const sellerId = await db.prepare('SELECT user_id FROM marketplace_listings WHERE id = ?').get(parseInt(action))?.user_id;
		...
		const title = await db.prepare('SELECT title FROM marketplace_listings WHERE id = ?').get(parseInt(action))?.title;
		await db.prepare("INSERT INTO notifications (recipient_id, actor_id, type, entity_type, entity_id, message) VALUES (?, ?, 'offer', 'listing', ?, ?)")
			.run(sellerId, userId, parseInt(action), `${userName} te ha ofrecido $${offerPrice} por tu artículo '${title}'.`);
...
export async function DELETE({ request, url, params }) {
	...
	if (parts[0] && /^\d+$/.test(parts[0])) {
		const result = await db.prepare('DELETE FROM marketplace_listings WHERE id = ? AND user_id = ?').run(parseInt(parts[0]), userId);
