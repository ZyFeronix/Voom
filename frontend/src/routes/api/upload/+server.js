...
const ALLOWED_MIMES = {
	...
	'audio/webm': 'webm', 'audio/mp4': 'mp4', 'audio/mpeg': 'mp3', 'audio/ogg': 'ogg',
	'video/mp4': 'mp4', 'video/webm': 'webm',
	'font/otf': 'otf', 'font/ttf': 'ttf', 'font/woff2': 'woff2', 'font/woff': 'woff',
	'application/x-font-opentype': 'otf', 'application/x-font-truetype': 'ttf', 'application/font-woff': 'woff'
};
...
export async function POST({ request }) {
	...
	if (file.size > maxBytes) return json({ error: `File exceeds maximum allowed size of ${maxMb}MB` }, { status: 400 });

	const mimeType = file.type;
	if (!ALLOWED_MIMES[mimeType]) return json({ error: 'Invalid file type. MIME detected: ' + mimeType }, { status: 400 });

	const ext = ALLOWED_MIMES[mimeType];
	const context = ['avatar', 'cover', 'chat', 'listing', 'post', 'font'].includes(formData.get('context')) ? formData.get('context') : 'chat';
