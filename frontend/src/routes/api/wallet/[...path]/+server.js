...
export async function _atomicTransfer(
	db,
	fromUserId,
	toUserId,
	amount,
	...
	const txn = db.transaction(async () => {
		const upd = await db.prepare(
			"UPDATE wallets SET balance = balance - ?, updated_at = datetime('now') WHERE user_id = ? AND balance >= ?"
		);
		...
		await db
			.prepare(
				'INSERT INTO transactions (user_id, wallet_id, type, amount, description, reference_id) VALUES (?, ?, ?, ?, ?, ?)'
			)
...
export async function POST({ request, _url, params }) {
	...
	if (action === 'transfer') {
		...
		try {
			await _atomicTransfer(
				db,
				userId,
				toUserId,
				amount,
				'transfer',
				body.description || 'Direct Transfer'
			);
	...
	if (action === 'tip') {
		...
		if (toUserId === userId)
			return json({ error: 'No puedes enviarte un tip a ti mismo' }, { status: 400 });
		try {
	...
	if (action === 'deposit') {
		...
		if (amount <= 0 || amount > 10000)
			return json({ error: 'Monto inválido (max $10,000)' }, { status: 400 });
		const txn = db.transaction(async () => {
			const upd = await db.prepare(
				"UPDATE wallets SET balance = balance + ?, updated_at = datetime('now') WHERE user_id = ?"
			);
			...
			await db
				.prepare(
					"INSERT INTO transactions (user_id, wallet_id, type, amount, description) VALUES (?, ?, 'deposit', ?, 'Depósito de saldo')"
				)
				.run(userId, w?.id, amount);
		});
		await txn();
	...
	if (action === 'withdraw') {
		...
		const txn = db.transaction(async () => {
			...
			await db
				.prepare(
					"INSERT INTO transactions (user_id, wallet_id, type, amount, description) VALUES (?, ?, 'withdraw', ?, 'Retiro de saldo')"
				)
				.run(userId, w?.id, -amount);
		});
		try {
			await txn();
			return json({ success: true, message: `$${amount} retirados de tu billetera` });
		} catch (e) {
			return json({ error: e.message }, { status: 400 });
		}
	}
