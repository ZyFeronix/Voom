...
export async function POST({ request, params }) {
	...
	if (action === 'offers' && !subaction) {
		...
		if (!listingId && !jobId)
			return json({ error: 'Must specify listing_id or job_id' }, { status: 400 });
		if (amount <= 0) return json({ error: 'Amount must be greater than zero' }, { status: 400 });

		let sellerId;
		if (listingId) {
			sellerId = await db.prepare('SELECT user_id FROM listings WHERE id = ?').get(listingId)
				?.user_id;
		} else {
		...
		if (sellerId === userId)
			return json({ error: 'Cannot make an offer on your own item' }, { status: 400 });
		...
		return json({
			...
			offer_id: Number(result.lastInsertRowid)
		});
	}
		...
		try {
			...
			const txn = db.transaction(async () => {
				...
				if (res.changes > 0) {
					...
					await db
						.prepare(
							"INSERT INTO transactions (user_id, wallet_id, type, amount, description, reference_id) VALUES (?, ?, 'fee', ?, 'Platform Commission', ?)"
						)
			...
			if (o.job_id)
				await db.prepare("UPDATE jobs SET status = 'closed' WHERE id = ?").run(o.job_id);
	...
	if (action === 'offers' && subaction === 'reject') {
		if (!offerId) return json({ error: 'Missing offer ID' }, { status: 400 });
		await db
			.prepare("UPDATE listing_offers SET status = 'rejected' WHERE id = ? AND seller_id = ?")
			.run(offerId, userId);
