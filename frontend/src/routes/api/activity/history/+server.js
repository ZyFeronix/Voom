		...
		const enrichedLogs = await Promise.all(logs.map(async log => {
			...
			if (log.entity_type === 'post' || log.entity_type === 'video') {
				if (['delete', 'update'].includes(log.action_type) && metadataObj?.previous_body) {
					...
					const post = await db.prepare('SELECT body, (SELECT COUNT(*) FROM post_media WHERE post_id = posts.id) as media_count FROM posts WHERE id = ?').get(log.entity_id);
					if (post) {
				...
				const reel = await db.prepare('SELECT caption FROM reels WHERE id = ?').get(log.entity_id);
				if (reel) {
				...
				if (['delete', 'update'].includes(log.action_type) && metadataObj?.previous_body) {
					...
					const comment = await db.prepare('SELECT body FROM comments WHERE id = ?').get(log.entity_id);
