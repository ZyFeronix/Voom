...
async function fetchPostMedia(db, postIds) {
	...
	const rows = await db.prepare(`SELECT post_id, id, media_type, media_url FROM post_media WHERE post_id IN (${placeholders})`).all(...postIds);
	const map = {};
	for (const m of rows) {
...
export async function GET({ request, url }) {
	...
	if (action === 'preferences') {
		const settings = await db.prepare(`SELECT feed_mode, w_interests, w_interactions, w_social, w_popularity, w_recency, w_diversity FROM user_settings WHERE user_id = ?`).get(userId);
		if (!settings) {
			return json({ preferences: { algorithm: 'intelligent', feed_mode: 'intelligent', w_interests: 50, w_interactions: 40, w_social: 30, w_popularity: 20, w_recency: 70, w_diversity: 15, weights: { interests: 50, interactions: 40, social: 30, popularity: 20, recency: 70, diversity: 15 } } });
		}
		return json({
			preferences: {
	...
	if (action === 'explore') {
		...
		if (posts.length > 0 && userId) {
			...
			const likedRows = await db.prepare(`SELECT post_id FROM post_reactions WHERE user_id = ? AND post_id IN (${placeholders})`).all(userId, ...postIds);
			const savedRows = await db.prepare(`SELECT post_id FROM saved_posts WHERE user_id = ? AND post_id IN (${placeholders})`).all(userId, ...postIds);
		...
		const nextCursor = posts.length === limit ? `${posts[posts.length - 1].like_count}_${posts[posts.length - 1].id}` : '';
	...
	if (!action) {
		...
		const settings = await db.prepare(`SELECT feed_mode, w_interests, w_interactions, w_social, w_popularity, w_recency, w_diversity FROM user_settings WHERE user_id = ?`).get(userId) || {
			feed_mode: 'intelligent', w_interests: 50, w_interactions: 40, w_social: 30, w_popularity: 20, w_recency: 70, w_diversity: 15
		};
		...
		if (mode === 'radar' || mode === 'chronological') {
			...
			// Intelligent or Retention
			let wSoc = settings.w_social / 100.0;
			let wPop = settings.w_popularity / 100.0;
			let wRec = settings.w_recency / 100.0;
			let wDiv = settings.w_diversity / 100.0;
			
			if (mode === 'retention') {
				// TikTok-like For You logic: Highly favors popularity and diversity
				wSoc = 0.2; wPop = 1.0; wRec = 0.8; wDiv = 0.8; 
			}
			...
			const sinceId = parseInt(url.searchParams.get('since_id')) || 0;
			const paginationClause = sinceId > 0 
				? `WHERE id > ?`
				: `WHERE (feed_score < ? OR (feed_score = ? AND id < ?))`;
			...
			const queryParams = [
				userId, userId, // user_liked, user_saved
				userId, userId, // scoreExpr (wSoc following check, wSoc self check)
				userId,         // scoreExpr (wDiv pseudo-random seed)
				userId,         // scoreExpr (liked penalty)
				userId, userId  // base filter (following check, self check)
			];
			if (sinceId > 0) {
			...
			nextCursor = posts.length === limit ? `${posts[posts.length - 1].feed_score}_${posts[posts.length - 1].id}` : '';
		}
...
export async function PUT({ request, url }) {
	...
	if (action === 'preferences') {
		...
		const db = getDb();
		const exists = await db.prepare(`SELECT user_id FROM user_settings WHERE user_id = ?`).get(userId);
		if (exists) {
			await db.prepare(`UPDATE user_settings SET feed_mode=?, w_interests=?, w_interactions=?, w_social=?, w_popularity=?, w_recency=?, w_diversity=?, updated_at=CURRENT_TIMESTAMP WHERE user_id=?`)
				.run(mode, w.interests ?? w.w_interests ?? 50, w.interactions ?? w.w_interactions ?? 40, w.social ?? w.w_social ?? 30, w.popularity ?? w.w_popularity ?? 20, w.recency ?? w.w_recency ?? 70, w.diversity ?? w.w_diversity ?? 15, userId);
		} else {
			await db.prepare(`INSERT INTO user_settings (user_id, feed_mode, w_interests, w_interactions, w_social, w_popularity, w_recency, w_diversity) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
				.run(userId, mode, w.interests ?? w.w_interests ?? 50, w.interactions ?? w.w_interactions ?? 40, w.social ?? w.w_social ?? 30, w.popularity ?? w.w_popularity ?? 20, w.recency ?? w.w_recency ?? 70, w.diversity ?? w.w_diversity ?? 15);
		}
