...
export async function GET({ request, url, params }) {
	...
	if (action && subaction === 'comments') {
		...
		await db
			.prepare(
				'CREATE TABLE IF NOT EXISTS reel_comment_likes (comment_id INTEGER, user_id INTEGER, PRIMARY KEY(comment_id, user_id))'
			)
			.run();
	...
	try {
		...
		if (subaction === 'like') {
			const existing = await db
				.prepare('SELECT 1 FROM reel_likes WHERE reel_id = ? AND user_id = ?')
				.get(reelId, userId);
			if (existing) return json({ success: true, message: 'Already liked' });
			await db
				.prepare('INSERT INTO reel_likes (reel_id, user_id) VALUES (?, ?)')
				.run(reelId, userId);
		...
		if (subaction === 'view') {
			...
			const existingView = db
				.prepare(
					`SELECT id FROM activity_logs WHERE user_id = ? AND action_type = 'view' AND entity_type = 'reel' AND entity_id = ? AND created_at >= datetime('now', '-1 day')`
				)
				.get(userId, reelId);
		...
		if (subaction === 'comments') {
			...
			if (commentId && commentAction === 'like') {
				...
				try {
					await db
						.prepare('INSERT INTO reel_comment_likes (comment_id, user_id) VALUES (?, ?)')
						.run(commentId, userId);
					await db
						.prepare('UPDATE reel_comments SET like_count = like_count + 1 WHERE id = ?')
						.run(commentId);
			...
			const replyToId =
				body.reply_to_id && !isNaN(body.reply_to_id) ? parseInt(body.reply_to_id) : null;
			...
			const result = await db
				.prepare(
					'INSERT INTO reel_comments (reel_id, user_id, body, parent_id, reply_to_id) VALUES (?, ?, ?, ?, ?)'
				)
				.run(reelId, userId, content, parentId, replyToId);
			await db
				.prepare('UPDATE reels SET comment_count = comment_count + 1 WHERE id = ?')
				.run(reelId);
			await logActivity(userId, 'create', 'comment', Number(result.lastInsertRowid), {
				reel_id: reelId
			});
			return json(
				{ success: true, message: 'Comment added', comment_id: result.lastInsertRowid },
				{ status: 201 }
			);
		}
		return json({ error: 'Endpoint not found' }, { status: 404 });
...
export async function DELETE({ request, _url, params }) {
	try {
		...
		if (subaction === 'like') {
			const result = await db
				.prepare('DELETE FROM reel_likes WHERE reel_id = ? AND user_id = ?')
				.run(reelId, userId);
			if (result.changes > 0) {
				await db
					.prepare('UPDATE reels SET like_count = MAX(like_count - 1, 0) WHERE id = ?')
					.run(reelId);
		...
		if (subaction === 'comments') {
			...
			if (commentAction === 'like') {
				const res = await db
					.prepare('DELETE FROM reel_comment_likes WHERE comment_id = ? AND user_id = ?')
					.run(commentId, userId);
				if (res.changes > 0) {
					await db
						.prepare('UPDATE reel_comments SET like_count = MAX(like_count - 1, 0) WHERE id = ?')
						.run(commentId);
			...
			if (isAdmin) {
				oldComment = await db
					.prepare('SELECT body FROM reel_comments WHERE id = ? AND reel_id = ?')
					.get(commentId, reelId);
				result = await db
					.prepare('DELETE FROM reel_comments WHERE id = ? AND reel_id = ?')
					.run(commentId, reelId);
			} else {
				oldComment = await db
					.prepare('SELECT body FROM reel_comments WHERE id = ? AND reel_id = ? AND user_id = ?')
					.get(commentId, reelId, userId);
				result = await db
					.prepare('DELETE FROM reel_comments WHERE id = ? AND reel_id = ? AND user_id = ?')
					.run(commentId, reelId, userId);
			}
			...
			if (result.changes > 0) {
				...
				await db
					.prepare('UPDATE reels SET comment_count = MAX(comment_count - ?, 0) WHERE id = ?')
