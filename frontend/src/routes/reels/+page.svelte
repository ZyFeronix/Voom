<script>
	...
	function trackReel(node, idx) {
		...
		return {
			update(newIdx) {
				reelElements[newIdx] = node;
			},
			destroy() {
				reelElements[idx] = null;
			}
		};
	}
	...
	async function deleteComment() {
		...
		try {
			...
			commentCountDeltas = {
				...commentCountDeltas,
				[reelId]: actualCount - (activeCommentReel.comment_count ?? 0)
			};
		} catch (_err) {
	...
	async function handleCommentSubmit() {
		...
		try {
			...
			commentCountDeltas = {
				...commentCountDeltas,
				[reelId]: (commentCountDeltas[reelId] ?? 0) + 1
			};
	...
	let activeCanvasLoops = new Map();

	function startCanvasLoop(mainVideo, canvas) {
		if (!mainVideo || !canvas) return;

		if (canvas.width !== 64) {
			canvas.width = 64;
			canvas.height = Math.floor(64 * (mainVideo.videoHeight / mainVideo.videoWidth)) || 114;
		}
		...
		function drawFrame() {
			if (mainVideo.paused || mainVideo.ended) {
				activeCanvasLoops.delete(mainVideo);
				return;
			}
			if (mainVideo.readyState >= 2) {
				ctx.drawImage(mainVideo, 0, 0, canvas.width, canvas.height);
			}
			activeCanvasLoops.set(mainVideo, requestAnimationFrame(drawFrame));
		}

		if (activeCanvasLoops.has(mainVideo)) {
			cancelAnimationFrame(activeCanvasLoops.get(mainVideo));
		}

		activeCanvasLoops.set(mainVideo, requestAnimationFrame(drawFrame));
	}

	function stopCanvasLoop(mainVideo) {
		if (activeCanvasLoops.has(mainVideo)) {
			cancelAnimationFrame(activeCanvasLoops.get(mainVideo));
			activeCanvasLoops.delete(mainVideo);
		}
	...
	function handleBackdropSync(e, action) {
		const mainVideo = e.target;
		const container = mainVideo.closest('.reel-item');
		if (!container) return;
		const canvas = container.querySelector('.backdrop-canvas');
		if (!canvas) return;

		if (action === 'play') {
			startCanvasLoop(mainVideo, canvas);
		} else if (action === 'pause') {
			stopCanvasLoop(mainVideo);
		} else if (action === 'sync_exact') {
			if (mainVideo.readyState >= 2) {
				if (canvas.width !== 64) {
					canvas.width = 64;
					canvas.height = Math.floor(64 * (mainVideo.videoHeight / mainVideo.videoWidth)) || 114;
...
<div class="tiktok-master-layout" class:comments-open={showCommentsModal}>
	...
	>
		<button
			onclick={toggleMute}
			class="aero-btn volume-btn"
			aria-label={globalVolume === 0 ? 'Activar sonido' : 'Silenciar'}
		>
						...
						<div class="reel-content-wrapper">
							...
							<div class="interaction-layer" onclick={(e) => handleVideoInteraction(e, reel, i)}>
								<video
									...
									onplay={(e) => {
										handlePlay(reel.id);
										handleBackdropSync(e, 'play');
									}}
									onpause={(e) => handleBackdropSync(e, 'pause')}
									onwaiting={(e) => handleBackdropSync(e, 'pause')}
									onplaying={(e) => handleBackdropSync(e, 'play')}
									onseeked={(e) => handleBackdropSync(e, 'sync_exact')}
								>
									<track kind="captions" />
								</video>
							...
							<div class="right-actions-sidebar">
								...
								<button
									class="clean-action-btn"
									class:liked={reel.user_liked}
								...
								<button class="clean-action-btn" onclick={(e) => toggleComments(reel.id, e)}>
									...
									<span class="action-label"
										>{(reel.comment_count ?? 0) + (commentCountDeltas[reel.id] ?? 0)}</span
									>
								</button>
							...
							{#snippet commentNode(node, depth = 0)}
								...
								<div class="comment-root" class:is-reply={depth > 0}>
									...
									<div class="comment-content">
										...
										<div class="body-text">
											{#if node.reply_to_username}
												<a href="/u/{node.reply_to_username}" class="mention-tag"
													>@{node.reply_to_username}</a
												>{' '}
											{/if}{node.body || node.comment}
										</div>
										<div class="comment-actions">
											<button class="reply-action" onclick={() => initiateReply(node)}
												>Responder</button
											>
											{#if authStore.user?.username === node.username || authStore.user?.role === 'admin' || authStore.user?.role === 'super_admin'}
												<button class="delete-action" onclick={() => requestDeleteComment(node.id)}
													>Eliminar</button
												>
											{/if}
										</div>
									...
									<div class="comment-likes">
										<button
											class="clean-action-btn like-comment-btn"
											onclick={() => handleCommentLike(node)}
										>
											<span
												class="material-icons-round"
												style="color: {node.has_liked ? '#e84a72' : '#888'}; font-size: {depth > 0
													? '16px'
													: '20px'};"
											>
						...
						<div class="comment-input-area">
							...
							>
								...
								<button
									...
									class="btn-aero-primary send-btn"
...
<style>
	...
	.global-error-toast {
		...
		z-index: 999;
		background: rgba(232, 74, 114, 0.95);
		...
		color: #fff;
		font-weight: 700;
		box-shadow: 0 10px 30px rgba(232, 74, 114, 0.4);
	}
	...
	.flash-icon .material-icons-round {
		font-size: 50px;
	}
	...
	.follow-badge .material-icons-round {
		font-size: 14px;
		font-weight: 900;
	}
	...
	.clean-action-btn .material-icons-round {
		font-size: 32px;
	...
	.clean-action-btn.liked .icon-outline {
		color: #f43f5e;
	...
	.floating-heart {
		...
		animation: heart-burst 1s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards;
	}
	...
	.comments-list-area::-webkit-scrollbar {
		width: 6px;
	}
	.comments-list-area::-webkit-scrollbar-track {
		background: transparent;
	}
	.comments-list-area::-webkit-scrollbar-thumb {
		background: rgba(255, 255, 255, 0.1);
		border-radius: 4px;
	}
	.comments-list-area::-webkit-scrollbar-thumb:hover {
	...
	.comment-root.is-reply {
		opacity: 0.95;
	}
	.comment-root.is-reply .comment-avatar {
		width: 28px;
		height: 28px;
		font-size: 0.8rem;
	}
