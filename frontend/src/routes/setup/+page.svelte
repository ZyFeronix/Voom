<script>
	...
	const canProceed = $derived(
		step === 1 ? agreed : step === 2 ? siteName.trim().length >= 2 : step === 3 ? step3Valid : false
	);
		...
		{#if step === 1}
			<div class="step-content animate-fade-in">
				...
				<div class="info-box">
					...
					<span
						>Este wizard solo aparece cuando no existen usuarios en el sistema. Una vez completado,
						no volverá a mostrarse.</span
					>
				</div>
			...
			<div class="step-content animate-fade-in">
				...
				<div class="form-group">
					...
					<div class="theme-options">
						<button
							class="theme-opt {defaultTheme === 'dark' ? 'selected' : ''}"
							onclick={() => (defaultTheme = 'dark')}
						>
							<span class="material-icons-round">dark_mode</span> Oscuro
						</button>
						<button
							class="theme-opt {defaultTheme === 'light' ? 'selected' : ''}"
