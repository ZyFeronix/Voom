<script>
	...
	onMount(() => {
		...
		// Carga diferida de vidstack (previews del compositor) en su propio chunk.
	...
	<div class="creator-layout">
		...
		<div class="tools-sidebar glass-panel">
			...
			<div class="tool-section mb-4">
				...
				<div
					class="textarea-wrapper glass-card p-3"
					style="border-radius: var(--radius-md); border: 1px solid var(--glass-border); background: var(--bg-overlay);"
				>
					<textarea
						id="post-body-input"
						bind:value={bodyText}
				...
				<div class="action-dock mt-3">
					...
					<button
						...
						class:pressed={showLocationPanel}
						aria-label="Action button"
					>
					...
					<button
						...
						class:pressed={showSchedulePanel}
						aria-label="Action button"
					>
				...
				{#if showEmojis}
					<div
						class="dropdown-panel animate-slide-in-up mt-3 relative"
						style="border-radius: var(--radius-md); border: 1px solid var(--border-subtle); background: var(--bg-surface2); width: 100%; display: flex; justify-content: center; overflow: hidden; padding: 0; z-index: 10;"
					>
						<TwemojiPicker
				...
				{#if showPollCreator}
					<div
						class="dropdown-panel glass-card aero-modal animate-slide-in-up mt-3 p-4 relative z-10"
					>
						<div class="panel-header mb-3 flex justify-between items-center">
							...
							<button
								class="panel-close w-7 h-7 rounded-full bg-surface hover:bg-rose-500/10 hover:text-rose-500 flex items-center justify-center transition border border-subtle"
								onclick={() => (showPollCreator = false)}
						...
						<input
							...
							class="vs-input mb-3 w-full aero-input"
						/>
						<div class="poll-options-list flex flex-col gap-2">
							{#each pollOptions as opt, i}
								<div class="poll-option-row flex gap-2 items-center">
									<span
										class="poll-num w-6 h-6 rounded-full bg-overlay border border-subtle flex items-center justify-center text-[10px] font-bold text-muted"
										>{i + 1}</span
									>
									<input
										type="text"
										...
										class="vs-input flex-1 aero-input"
									/>
									{#if pollOptions.length > 2}
										<button
											class="poll-remove-btn w-6 h-6 flex items-center justify-center text-muted hover:text-rose-500 transition"
						...
						{#if pollOptions.length < 6}
							<button
								class="poll-add-btn mt-3 w-full py-2 border border-dashed border-subtle rounded-xl text-muted text-sm font-bold hover:text-sky-400 hover:border-sky-400 transition hover:bg-sky-500/5 flex items-center justify-center gap-1"
								onclick={addPollOption}
								aria-label="Action button"
								><span class="material-icons-round text-sm">add</span> Añadir Opción</button
							>
						{/if}
						<div
							class="poll-duration-wrap mt-4 pt-3 border-t border-subtle flex items-center justify-between"
						>
							<span class="text-xs font-bold text-muted uppercase tracking-wider">Duración</span>
							<select
								bind:value={pollDuration}
								class="vs-input aero-input py-1 px-3 w-auto text-sm"
							>
				...
				{#if showLocationPanel}
					...
					>
						<div class="panel-header mb-3 flex justify-between items-center">
							...
							<button
								class="panel-close w-7 h-7 rounded-full bg-surface hover:bg-rose-500/10 hover:text-rose-500 flex items-center justify-center transition border border-subtle"
								onclick={() => (showLocationPanel = false)}
								><span class="material-icons-round text-sm">close</span></button
							>
						</div>
						<input
				...
				{#if showSchedulePanel}
					...
					>
						...
						<div class="flex flex-col gap-3">
							<label
								class="aero-switch cursor-pointer flex items-center gap-3 p-2 rounded-xl hover:bg-surface transition"
							>
								<input type="checkbox" bind:checked={isScheduled} class="hidden-peer" />
								<span
									class="switch-slider w-10 h-5 rounded-full bg-overlay border border-subtle relative transition-colors duration-300 before:content-[''] before:absolute before:w-3 before:h-3 before:bg-white before:rounded-full before:top-[3px] before:left-[3px] before:transition-transform"
								></span>
							...
							{#if isScheduled}
								<div
									class="flex flex-col gap-1 mt-2 p-3 bg-overlay border border-subtle rounded-xl"
								>
									<span class="text-[10px] font-bold text-muted uppercase tracking-wider mb-1"
										>Fecha y Hora de Publicación</span
									>
									<input
										type="datetime-local"
			...
			<div class="tool-section mb-4">
				...
				>
					...
					<p class="dropzone-text">
						Arrastra imágenes/videos o <span class="text-aero-blue">haz clic</span>
					</p>
				</div>
				<input
					type="file"
			...
			<!-- Footer Actions -->
			<div
				class="create-footer mt-5 pt-4 border-t border-glass-border flex justify-between items-center gap-3"
			>
				...
				<button
					class="btn-aero-primary px-6 publish-btn"
		...
		<div class="preview-section">
			<div class="preview-card-wrapper max-w-md w-full animate-slide-in-up">
				<div
					class="preview-label font-bold text-xs uppercase tracking-wider text-muted mb-4 text-center"
				>
				...
				<article class="preview-post-card" aria-label="Link">
					...
					<div class="flex items-start justify-between mb-3">
						<div class="flex items-center gap-3">
							...
							{#if authStore.user?.avatar_url}
								<div
									class="w-11 h-11 rounded-full overflow-hidden border border-white/20 flex-shrink-0"
								>
									<img
							...
							{:else}
								<div
									class="w-11 h-11 rounded-full bg-gradient-to-br from-cyan-400 to-blue-500 flex items-center justify-center border border-white/20 flex-shrink-0"
								>
									<span class="text-white font-bold"
										>{(authStore.user?.username || '?')[0].toUpperCase()}</span
									>
								</div>
							{/if}
							...
							<div>
								...
								<div class="flex flex-col">
									<span class="text-[10px] text-muted"
										>@{authStore.user?.username || 'usuario'} • Hace un momento</span
									>
									{#if locationName}
										<span
											class="text-[10px] text-sky-400 font-semibold flex items-center gap-0.5 mt-0.5"
										>
					...
					{:else}
						<p class="post-body text-sm text-muted my-3 italic">
							Escribe algo en la caja de texto para verlo aquí...
						</p>
					{/if}
					...
					{#if showPollCreator && pollQuestion.trim()}
						<div
							class="poll-widget-container p-4 rounded-xl mb-4 bg-surface/30 border border-subtle max-w-sm"
						>
							...
							<div class="flex flex-col gap-2">
								{#each pollOptions.filter((o) => o.trim()) as option}
									<div
										class="poll-option-btn w-full p-2.5 rounded-lg border border-subtle bg-surface/50 text-left font-semibold text-xs text-main flex justify-between items-center"
									>
										<span>{option}</span>
					...
					{#if selectedFiles.length > 0 || uploadedMedia.length > 0}
						<div
							class="grid gap-2 rounded-xl overflow-hidden mt-3 max-h-[300px]"
							style="grid-template-columns: {selectedFiles.length + uploadedMedia.length > 1
								? '1fr 1fr'
								: '1fr'};"
						>
							{#each uploadedMedia as media}
								<div class="relative aspect-video bg-neutral-900 flex items-center justify-center">
									...
									{#if media.klipy}
										<span
											class="absolute bottom-2 left-2 px-1.5 py-0.5 rounded bg-black/60 text-[10px] text-white font-bold tracking-wider"
											>GIF</span
										>
									{/if}
								</div>
							...
							{#each selectedFiles as file}
								<div class="relative aspect-video bg-neutral-900 flex items-center justify-center">
									{#if file.type.startsWith('video/')}
										...
										<span class="absolute top-2 right-2 bg-black/60 p-1 rounded-full"
											><span class="material-icons-round text-white text-xs">videocam</span></span
										>
									{:else}
										<img
											src={URL.createObjectURL(file)}
					...
					<!-- Mock Actions -->
					<div
						class="action-bar flex justify-between border-t border-subtle pt-3 mt-4 text-xs text-muted font-semibold"
					>
						<span class="flex items-center gap-1 cursor-pointer hover:text-rose-500 transition"
							><span class="material-icons-round text-sm">favorite_border</span> 0</span
						>
						<span class="flex items-center gap-1 cursor-pointer hover:text-blue-500 transition"
							><span class="material-icons-round text-sm">chat_bubble_outline</span> 0</span
						>
						<span class="flex items-center gap-1 cursor-pointer hover:text-green-500 transition"
							><span class="material-icons-round text-sm">share</span> Compartir</span
						>
						<span class="flex items-center gap-1 cursor-pointer hover:text-amber-500 transition"
							><span class="material-icons-round text-sm">bookmark_border</span> Guardar</span
						>
					</div>
				</article>
			</div>
...
<style>
	...
	.mood-scroller.dragging .mood-pill {
		pointer-events: none;
	}
	...
	.mood-pill.selected {
		...
		box-shadow: 0 4px 14px rgba(74, 171, 223, 0.3);
		color: var(--aero-sky);
		transform: translateY(-2px);
	}
	.m-icon {
	...
	.dock-btn.pressed {
		...
		box-shadow:
			0 8px 24px rgba(74, 171, 223, 0.25),
			inset 0 0 16px rgba(74, 171, 223, 0.1);
		transform: translateY(-2px);
	}
	.dock-btn:active {
	...
	/* ── Icon accent colors ── */
	.dock-btn .dock-icon.amber {
		color: #fbbf24;
	}
	.dock-btn .dock-icon.fuchsia {
		color: #e879f9;
	}
	.dock-btn .dock-icon.emerald {
		color: #34d399;
	}
	.dock-btn .dock-icon.sky {
		color: #38bdf8;
	}
	.dock-btn .dock-icon.indigo {
		color: #818cf8;
	}

	.dock-btn.pressed .dock-icon.amber {
		background: rgba(251, 191, 36, 0.12);
		border-color: rgba(251, 191, 36, 0.3);
		box-shadow: 0 0 10px rgba(251, 191, 36, 0.2);
	}
	.dock-btn.pressed .dock-icon.fuchsia {
		background: rgba(232, 121, 249, 0.12);
		border-color: rgba(232, 121, 249, 0.3);
		box-shadow: 0 0 10px rgba(232, 121, 249, 0.2);
	}
	.dock-btn.pressed .dock-icon.emerald {
		background: rgba(52, 211, 153, 0.12);
		border-color: rgba(52, 211, 153, 0.3);
		box-shadow: 0 0 10px rgba(52, 211, 153, 0.2);
	}
	.dock-btn.pressed .dock-icon.sky {
		background: rgba(56, 189, 248, 0.12);
		border-color: rgba(56, 189, 248, 0.3);
		box-shadow: 0 0 10px rgba(56, 189, 248, 0.2);
	}
	.dock-btn.pressed .dock-icon.indigo {
