<script>
	...
	let selfTypingTimeout = null; // debounce del evento "dejé de escribir" propio
	...
	// Presencia en vivo: una vez conectados al socket, la fuente de verdad es
	// el store de presencia; antes de conectar usamos el valor inicial de la API.
	...
	// "Desconectado · visto hace X" a partir de peer_last_seen.
				...
				if (!processedSignalIds.has(sig.id)) {
					...
					try {
						...
						if (payload.type === 'offer' && !inCall) {
							...
							rtcManager &&
							Number(sig.conversation_id) === Number(rtcManager.conversationId)
						) {
							rtcManager.handleSignal(sig.sender_id, payload);
						} else if (
							Number(sig.conversation_id) === Number(activeConvId) ||
							incomingCallOffer?.conversation_id === sig.conversation_id
						) {
	...
	onMount(async () => {
		...
		const handleTyping = ({ convId, userId, isTyping }) => {
			...
			if (Number(userId) === Number(authStore.user?.id)) return; // ignorar el propio
			if (isTyping) {
				showTypingIndicator();
			} else {
		...
		if (sock) sock.on('typing', handleTyping);

		return () => {
			window.removeEventListener('message_reaction', handleMessageReaction);
			if (sock) sock.off('typing', handleTyping);
			const s = notificationsStore.getSocket?.() ?? null;
			if (s && joinedConvId) s.emit('leave_conversation', joinedConvId);
		};
	});
	...
	async function selectConversation(id) {
		...
		// Unirse a la sala de la conversación en el socket para recibir "typing"
		// en tiempo real (dejando la anterior si la había).
		const sock = notificationsStore.getSocket?.() ?? null;
		if (sock) {
	...
	async function sendMessage(e) {
		...
		if ((!messageText.trim() && !attachedFileUrl && !attachedVoiceNote) || sending || !activeConvId)
			return;
	...
	function handleKeydown(e) {
		if (e.key !== 'Enter' && activeConvId) {
			const sock = notificationsStore.getSocket?.() ?? null;
			if (!sock) return;
			// Emitir "escribiendo" y programar el "dejé de escribir" tras 2s de pausa.
								...
								<div class="conv-meta">
									...
									{#if conv.last_message_time}
										<span class="conv-time">
											{new Date(conv.last_message_time).toLocaleTimeString('es', {
												hour: '2-digit',
												minute: '2-digit'
											})}
										</span>
		...
		<div class="chat-pane" class:hidden-mobile={mobileView === 'list'}>
			{#if activeConvId}
				...
				<div class="chat-header">
					<div class="peer-profile-info">
						<!-- Mobile back button -->
						<button onclick={() => (mobileView = 'list')} class="back-mobile-btn md:hidden">
							...
							<div class="conv-avatar" style="width: 38px; height: 38px;">
								{#if activeConv.peer_avatar}
									<img
										...
										alt={activeConv.name || activeConv.peer_display_name}
										width="38"
										height="38"
										loading="lazy"
										decoding="async"
						...
						<div style="min-width: 0;">
							...
							<p class="peer-status" class:typing-status={isPeerTyping}>
								{#if isPeerTyping}
									Escribiendo...
								{:else if peerOnline(activeConv)}
									Activo ahora
								{:else}
									{lastSeenLabel(activeConv)}
								{/if}
							</p>
						</div>
					</div>
					...
					<div class="flex items-center gap-2">
						{#if inCall}
							...
							<button
								class="aero-icon-btn !w-9 !h-9"
								style="background: linear-gradient(135deg,#ef4444,#dc2626); color:#fff; border-color:rgba(255,255,255,.15);"
								title="Colgar"
								onclick={endCall}
							>
						...
						{:else}
							<!-- Audio Call -->
							<button
								class="aero-icon-btn !w-9 !h-9 text-main bg-transparent hover:bg-white/10"
						...
						{#if activeConv.peer_username}
							<a
								href="/u/{activeConv.peer_username}"
								class="btn-aero-secondary"
								style="padding: 6px 12px; font-size: 0.75rem; text-decoration: none;"
								aria-label="Link">Ver Perfil</a
							>
						{/if}
					</div>
				</div>
				...
				>
					{#if hasMoreMessages && !loadingMoreMessages}
						<button class="load-more-btn" onclick={loadMoreMessages} aria-label="Action button">
							...
							{#if messages.length === 0 && !loadingMsgs}
								<div class="empty-chat">
									...
									<p class="empty-chat-subtitle">
										Envía un mensaje para comenzar a chatear con {activeConv.peer_display_name ||
												...
												{#if !isMe}
													<div class="peer-mini-avatar">
														...
														{:else}
															<span
																>{getInitials(
																	activeConv.peer_display_name || activeConv.peer_username
																)}</span
															>
														{/if}
													</div>
													...
													>
														...
														{:else}
															...
															{#if msg.body || msg.content}
																...
																<div
																	class="message-actions-wrapper {isMe
																		? 'actions-right'
																		: 'actions-left'}"
																>
																...
																<p class="message-text-p" use:twemojiAction>
														...
														<div class="delete-confirm-inline" in:fade={{ duration: 150 }}>
															...
															<div class="flex gap-2">
																<button
																	...
																	onclick={() => deleteMessage(msg.id)}>Sí, borrar</button
																>
																<button
																	class="btn-cancel-del"
																	onclick={() => (deletingMessageId = null)}>Cancelar</button
																>
															</div>
														</div>
													{/if}
													...
													<span class="msg-time">
														...
														{#if isMe && msg.read_at}
															<span
																...
																title="Visto {new Date(msg.read_at).toLocaleTimeString('es', {
																	hour: '2-digit',
																	minute: '2-digit'
																})}"
															>
			...
			{:else}
				...
				<div class="no-chat-selected">
					...
					<p class="peer-status" style="max-w: 280px; margin: 0 auto 16px auto;">
						Selecciona un chat de la lista izquierda o crea uno nuevo para empezar a hablar en
						tiempo real.
					</p>
					<button
						onclick={() => (showNewDMModal = true)}
...
{#if showNewDMModal}
	...
	>
		<div
			...
			style="max-width: 360px; width: 100%; padding: 20px;"
			...
			<div class="modal-results">
				...
				{:else}
					{#each userSearchResults as user}
						<button onclick={() => startDMWithUser(user)} class="modal-user-item">
	...
	<div class="rtc-modal-overlay" style="z-index: 10000;" transition:fade={{ duration: 200 }}>
		...
		>
			...
			<div class="flex flex-col gap-4">
				...
				<div class="flex flex-col gap-3">
					{#if availableCams.length > 0 && pendingCallType === 'video'}
						<div class="flex flex-col gap-1">
							...
							<select
								id="cam-select"
								class="aero-input w-full !bg-black/20 !border-white/10 !text-white"
								bind:value={selectedCamId}
								onchange={updatePreview}
							>
								{#each availableCams as cam}
									<option class="text-black" value={cam.deviceId}
										>{cam.label || `Cámara ${cam.deviceId.slice(0, 5)}`}</option
									>
								{/each}
							</select>
						</div>
						...
						<div class="flex flex-col gap-1">
							...
							>
								{#each availableMics as mic}
									<option class="text-black" value={mic.deviceId}
										>{mic.label || `Micrófono ${mic.deviceId.slice(0, 5)}`}</option
									>
								{/each}
							</select>
						</div>
				...
				<div class="mt-2">
					<button
						class="btn-aero-primary w-full py-3 flex items-center justify-center gap-2"
						onclick={confirmDeviceSetup}
						aria-label="Action button"
					>
	...
	<div class="rtc-modal-overlay" transition:fade={{ duration: 200 }}>
		<div class="rtc-modal-glass" transition:scale={{ duration: 300, easing: backOut, start: 0.9 }}>
			...
			<div class="rtc-header">
				...
				<button
					class="aero-icon-btn !w-7 !h-7 bg-transparent hover:bg-white/10 text-main"
					title="Minimizar"
				...
				{:else if callType === 'video' && localStream}
					<div class="rtc-video-wrapper rtc-self-cam">
						<video
							...
							playsinline
							class="rtc-video"
							style={camMuted ? 'filter:brightness(0.2)' : ''}
						></video>
						<span class="rtc-label">Tú</span>
						{#if camMuted}<div class="rtc-cam-off-overlay">
				...
				{:else}
					...
					<div class="rtc-audio-pulse">
						<div
							...
							style={micMuted ? 'border-color:rgba(255,100,100,.4)' : ''}
						>
							{#if activeConv?.peer_avatar}
								<img
									src={activeConv.peer_avatar}
						...
						<p class="text-sm font-semibold text-main mt-2">
							{activeConv?.peer_display_name || activeConv?.peer_username || 'Participante'}
						</p>
						{#if remoteStreams.length === 0}
				...
				{#each remoteStreams as rs (rs.peerId)}
					<div class="rtc-video-wrapper">
						...
						<span class="rtc-label"
							>{activeConv?.peer_display_name || activeConv?.peer_username || 'Participante'}</span
						>
					</div>
				{/each}
			</div>
...
<style>
	...
	.rtc-avatar-ring {
		...
		box-shadow:
			...
			0 0 0 16px rgba(0, 229, 255, 0.03);
	}
	...
	@keyframes pulseGlow {
		...
		50% {
			...
			box-shadow:
				...
				0 0 0 22px rgba(0, 229, 255, 0.05);
		}
	}
	...
	.chat-composer:focus-within {
		...
		box-shadow: 0 0 0 4px rgba(46, 134, 232, 0.1);
	...
	.composer-send-btn:hover:not(:disabled) {
		...
		box-shadow: 0 8px 22px rgba(27, 133, 243, 0.5);
	}
