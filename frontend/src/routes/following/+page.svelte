<script>
	...
	let initialTab = page.url.searchParams.get('tab');
	let activeTab = $state(
		initialTab && ['verified', 'followers', 'following'].includes(initialTab)
			? initialTab
			: 'following'
	);
	...
	onMount(async () => {
		...
		if (currentTab && ['verified', 'followers', 'following'].includes(currentTab)) {
	...
	async function loadNetworkData() {
		...
		try {
			const [followersData, followingData] = await Promise.all([
				usersApi.followers('me'),
				usersApi.following('me')
			]);

			followers = (followersData.followers || []).map((u) => ({
				...u,
				is_following: u.is_following === 1 || u.is_following === true || u.is_following === '1'
			}));

			following = (followingData.following || []).map((u) => ({
				...u,
				is_following: true
			}));
		} catch (e) {
			console.error('[Following] Error al cargar datos de red:', e);
			error = e.message || 'No se pudo cargar la lista de conexiones.';
		} finally {
			loading = false;
		}
	}
	...
	async function toggleFollow(targetUser, e) {
		...
		// Añadir a conjunto de bloqueo para evitar clicks múltiples concurrentes
		...
		// Actualizar en ambas listas si el usuario está presente en ambas
		followers = followers.map((u) =>
			u.id === targetId ? { ...u, is_following: !currentlyFollowing } : u
		);

		if (currentlyFollowing) {
			// Si dejó de seguirlo y estamos en la pestaña 'following', no lo removemos inmediatamente
			// para que no salte abruptamente la UI, pero marcamos el estado.
			following = following.map((u) => (u.id === targetId ? { ...u, is_following: false } : u));
		} else {
			// Si lo siguió de nuevo desde seguidores, asegurarnos de que aparezca
			const existsInFollowing = following.some((u) => u.id === targetId);
			if (!existsInFollowing) {
				following = [{ ...targetUser, is_following: true }, ...following];
			} else {
				following = following.map((u) => (u.id === targetId ? { ...u, is_following: true } : u));
			}
		}
		...
		try {
			...
			console.error('[Following] Error en toggle follow:', err);
	...
	<header class="network-header glass-panel">
		...
		<div class="header-info">
			<h1 class="header-name">
				<span>Mi Red</span>
				{#if authStore.user && hasBadge(authStore.user)}
					<VerifiedBadge
						role={authStore.user.role || 'user'}
						isVerified={authStore.user.is_verified == 1 ||
							authStore.user.is_verified === true ||
							authStore.user.is_verified === '1'}
						size="18px"
						interactive={true}
					/>
				{/if}
			</h1>
			<span class="header-handle">@{authStore.user?.username || 'usuario'}</span>
		</div>
	</header>
	...
	<main class="network-list">
		...
		{:else if error}
			<div class="state-card error-state glass-card" in:fade={{ duration: 180 }}>
				...
				<h3 class="state-title">Error al sincronizar tu red</h3>
				<p class="state-desc">{error}</p>
				<button class="btn-aero-primary retry-btn" onclick={loadNetworkData}>
					...
					{#if filteredList.length === 0}
						<div class="state-card empty-state glass-card">
							...
							{:else if activeTab === 'following'}
								...
								<h3 class="state-title">Aún no sigues a nadie</h3>
								<p class="state-desc">
									Descubre ilustradores, VTubers y comunidades virtuales para llenar tu feed.
								</p>
								<a href="/explore" class="btn-aero-primary action-btn">
							...
							{:else if activeTab === 'followers'}
								...
								<h3 class="state-title">Aún no tienes seguidores</h3>
								<p class="state-desc">
									Comparte publicaciones y reels para conectar con nuevas audiencias.
								</p>
								<a href="/posts/create" class="btn-aero-primary action-btn">
							...
							{:else}
								...
								<p class="state-desc">
									Cuando creadores verificados o miembros oficiales del staff formen parte de tu
									red, aparecerán destacados aquí.
								</p>
								<a href="/about/verified" class="btn-aero-secondary action-btn">
									<span class="material-icons-round">info</span>
									<span>Conocer sobre Verificaciones</span>
								</a>
							{/if}
						</div>
					{:else}
...
<style>
	/* ════════════════════════════════════════════════════════════════════════
	   NETWORK PAGE (FOLLOWING / FOLLOWERS / VERIFIED) — NEO-AERO GLASS 2.0
	   ════════════════════════════════════════════════════════════════════════ */
