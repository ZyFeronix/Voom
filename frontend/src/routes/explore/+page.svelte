<script>
  ...
  import { feed as feedApi, users as usersApi, marketplace as marketplaceApi, reels as reelsApi } from '$lib/api.js';
    ...
    try {
      if (searchQuery.trim()) {
        ...
        if (activeTab === 'posts' || activeTab === 'todo') {
          const feedRes = await feedApi.explore({ q: searchQuery, category: activeCategory !== 'all' ? activeCategory : undefined });
          posts = feedRes.posts || [];
        }
        ...
        // Normal explore mode
        const feedRes = await feedApi.explore({ category: activeCategory !== 'all' ? activeCategory : undefined });
        posts = feedRes.posts || [];

        // Load suggested creators
        const userRes = await usersApi.suggestedCreators();
...
<div class="explore-container">
  ...
  <div class="glass-card search-header">
    ...
    <div class="search-header-inner">
      ...
      <p class="header-subtitle">Busca VTubers, posts de tus temas favoritos, assets de diseño y más.</p>
    ...
    <div class="results-container">
                ...
                <a href="/u/{creator.username}" class="creator-info">
                  ...
                  <div class="creator-details">
                    ...
                    {#if creator.is_vtuber}
                      <span class="aero-badge-virtual" style="margin-top: 4px; align-self: flex-start;">VTuber</span>
                    {/if}
                  </div>
                </a>
                <button
                ...
                <div class="glass-card market-card">
                  <div class="market-media">
                    {#if item.media_url || item.image_url}
                      <img src={item.media_url || item.image_url} alt={item.title} class="market-img" />
                    {:else}
                  ...
                  <div class="market-info">
                    ...
                    <div class="market-footer">
                      ...
                      <a href="/marketplace" class="btn-aero-secondary" style="padding: 4px 10px; font-size: 0.75rem; text-decoration: none;">Ver oferta</a>
                    </div>
                  </div>
                </div>
              {/each}
      ...
      {#if creators.length === 0 && posts.length === 0 && marketplaceItems.length === 0 && reelsList.length === 0}
        <div class="glass-card empty-state">
          ...
          <p>No pudimos encontrar nada que coincida con tu búsqueda. Revisa la ortografía o intenta buscar otro término.</p>
        </div>
      {/if}
