<script>
  ...
  const requirementsAllOk = $derived(
    requirements.node_ok && requirements.sqlite_ok && requirements.uploads_writable && requirements.db_writable
  );
</script>
  ...
  <div class="install-card glass-panel">
    ...
    {#if step === 1}
      <div class="step-content">
        ...
        <p class="description">Verificando la compatibilidad de tu entorno para ejecutar VSocial.</p>
        ...
        {:else}
          <div class="requirements-list">
            ...
            <div class="req-item flex justify-between items-center">
              ...
              <span class="material-icons-round icon-{requirements.uploads_writable ? 'ok' : 'fail'}">
          ...
          <div class="actions flex justify-between items-center mt-6">
            ...
            <button
              onclick={() => { if (requirementsAllOk) step = 2; }}
    ...
    {#if step === 2}
      <div class="step-content">
        ...
        <div class="form-group">
          ...
          <input type="text" id="sitename" bind:value={siteName} class="aero-input w-full" placeholder="Ej: VSocial" />
        </div>

        <div class="form-group flex items-center justify-between gap-4 mt-4">
          <div>
            <label for="allow_reg" style="margin: 0; font-weight: 700;">Permitir Registro Abierto</label>
            <span class="text-xs text-muted block">Permite que nuevos usuarios se registren de forma pública.</span>
          </div>
          <input type="checkbox" id="allow_reg" bind:checked={allowRegistration} class="aero-toggle-switch" />
        </div>

        <div class="actions flex justify-between items-center mt-8">
          <button onclick={() => step = 1} class="btn-aero-secondary">
            Atrás
          </button>
    ...
    {#if step === 3}
      <form onsubmit={handleInstall} class="step-content">
        ...
        <div class="form-group">
          ...
          <input type="text" id="admin_user" bind:value={adminUsername} class="aero-input w-full" required />
        </div>

        <div class="form-group mt-3">
          <label for="admin_email">Correo Electrónico</label>
          <input type="email" id="admin_email" bind:value={adminEmail} class="aero-input w-full" required />
        </div>

        <div class="form-group mt-3">
          <label for="admin_pass">Contraseña</label>
          <input type="password" id="admin_pass" bind:value={adminPassword} class="aero-input w-full" required minlength="6" placeholder="Mínimo 6 caracteres" />
        </div>

        <div class="actions flex justify-between items-center mt-8">
          <button type="button" onclick={() => step = 2} class="btn-aero-secondary" disabled={installing}>
            Atrás
          </button>
    ...
    {#if step === 4 && installSuccess}
      <div class="step-content text-center py-6">
        ...
        <p class="description mt-2">VSocial se ha configurado e instalado correctamente con una base de datos local SQLite y contenido demo de VTubers.</p>
        ...
        <div class="admin-summary glass-panel p-4 mt-6 text-left">
          ...
          <p class="text-sm"><strong>Contraseña:</strong> <em>La que ingresaste en el formulario</em></p>
          <p class="text-xs text-muted mt-3">Demo VTubers agregados para pruebas: <strong>sakura_nova</strong>, <strong>neon_drifter</strong> (Contraseña: <em>password123</em>)</p>
        </div>
...
<style>
  ...
  .install-logo { display: flex; flex-direction: column; }
  ...
  .steps-indicator { display: flex; align-items: center; gap: 4px; }
  ...
  .step-line.active { background: var(--color-aero-cyan); }
  ...
  .icon-ok { color: #00E676; }
  .icon-fail { color: #FF1744; }
  ...
  :global(:root) {
    ...
    --bubble-bg: radial-gradient(circle at 30% 30%, rgba(255,255,255,0.9), rgba(0, 229, 255, 0.25));
