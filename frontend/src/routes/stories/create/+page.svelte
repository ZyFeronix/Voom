<script>
	...
	let textAlign = $state('center'); // 'left' | 'center' | 'right'
	let textPosX = $state(50);
	let textPosY = $state(80);
	...
	// ── Derived ───────────────────────────────────────────────────────────────
	const canPublish = $derived((selectedFile !== null || caption.trim().length > 0) && !uploading);

	const currentFont = $derived(FONTS.find((f) => f.id === selectedFont) ?? FONTS[0]);

	const textOverlayStyle = $derived(
		`top: ${textPosY}%; left: ${textPosX}%; transform: translate(-50%, -50%); ` +
			`color: ${textColor}; text-align: ${textAlign}; ${currentFont.style}` +
			(textBg
	...
	function onTextPointerDown(e) {
		if (!phoneFrameNode) return;
		isDraggingText = true;
		e.target.setPointerCapture(e.pointerId);
	}
	function onTextPointerMove(e) {
		if (!isDraggingText || !phoneFrameNode) return;
		const rect = phoneFrameNode.getBoundingClientRect();
		let x = ((e.clientX - rect.left) / rect.width) * 100;
		let y = ((e.clientY - rect.top) / rect.height) * 100;
		textPosX = Math.max(0, Math.min(100, x));
		textPosY = Math.max(0, Math.min(100, y));
	}
	...
	async function handlePublish() {
		...
		try {
			...
			// Enviamos los metadatos de texto para renderizado fiel en el viewer
			fd.append(
				'text_meta',
				JSON.stringify({
					color: textColor,
...
<div class="sc-root">
	...
	<header class="sc-topbar glass-panel">
		...
		<!-- Indicador de estado (sin botón duplicado) -->
		<div class="sc-topbar-status">
			{#if uploading}
				<span class="sc-spinner" style="width:18px;height:18px;"></span>
			{:else if success}
				<span class="material-icons-round" style="color:#00e464;">check_circle</span>
			{:else if canPublish}
				<span class="sc-topbar-ready">Listo</span>
			{/if}
		</div>
	</header>
	...
	<div class="sc-body">
		...
		<aside class="sc-tools glass-panel">
			<section class="sc-section">
				...
				{:else}
					<div class="sc-file-chip">
						<span class="material-icons-round sc-fc-icon"
							>{previewType === 'video' ? 'videocam' : 'image'}</span
						>
			...
			<section class="sc-section">
				...
				{#if caption.trim()}
						...
						<div class="sc-font-chips">
							{#each FONTS as f}
								<button
									...
									onclick={() => (selectedFont = f.id)}>{f.label}</button
								>
							{/each}
						</div>
					</div>
					...
					<!-- Alineación de texto -->
					<div class="sc-subsection">
						<p class="sc-sub-label">Alineación</p>
						<div class="sc-align-row">
							{#each [['left', 'format_align_left'], ['center', 'format_align_center'], ['right', 'format_align_right']] as [a, icon]}
								<button
									class="sc-align-btn"
				...
				{:else}
					...
					{#if preview}
						{#if previewType === 'video'}
							<video src={preview} class="sc-backdrop" muted loop autoplay playsinline></video>
							<MediaPlayer
								src={preview}
								type="video"
					...
					{#if caption.trim()}
						<div
							class="sc-text-overlay {isDraggingText ? 'dragging' : ''}"
		...
		<aside class="sc-info glass-panel">
			...
			<p class="sc-disclaimer">
				Tu historia desaparecerá automáticamente en 24 horas. No aparecerá en tu perfil permanente.
			</p>
...
<style>
	...
	.sc-badge-24h .material-icons-round {
		font-size: 12px;
	}

	/* Topbar status indicator (replaces the duplicate publish button) */
	.sc-topbar-status {
		display: flex;
		align-items: center;
	...
	.sc-section-title .material-icons-round {
		font-size: 14px;
		color: var(--aero-blue);
	}
	...
	.sc-fc-remove .material-icons-round {
		font-size: 16px;
	}
	...
	.sc-color-dot.active {
		border-color: #fff;
		box-shadow: 0 0 0 2px var(--aero-blue);
		transform: scale(1.1);
	}
	...
	.sc-align-btn .material-icons-round {
		font-size: 18px;
	}
	...
	.sc-preview-area {
		...
		background: radial-gradient(
			ellipse at center,
			rgba(var(--accent-blue-rgb), 0.04) 0%,
			transparent 70%
		);
		overflow: hidden;
	}
	...
	.sc-text-overlay {
		...
		cursor: grab;
		touch-action: none; /* para que funcione pointer events en móviles */
		user-select: none;
		animation: scTextIn 0.35s cubic-bezier(0.34, 1.56, 0.64, 1);
	}
	.sc-text-overlay:hover {
		outline: 2px dashed rgba(255, 255, 255, 0.5);
		outline-offset: 4px;
		border-radius: var(--radius-xs);
	}
	.sc-text-overlay.dragging {
		cursor: grabbing;
		outline: 2px solid var(--aero-blue);
	...
	.sc-empty-icon .material-icons-round {
		font-size: 2.2rem;
		color: #fff;
	}
	.sc-empty-hint {
	...
	.sc-duration-pill .material-icons-round {
		font-size: 11px;
	}
	...
	.sc-remove-media:hover {
		background: rgba(232, 74, 114, 0.7);
		transform: scale(1.08);
	}
	.sc-remove-media .material-icons-round {
		font-size: 16px;
	}
	...
	.sc-success .material-icons-round {
		font-size: 18px;
		flex-shrink: 0;
	}
	...
	.sc-submit:not(:disabled):hover {
		...
		box-shadow: 0 8px 28px rgba(var(--accent-blue-rgb), 0.45);
	}
	.sc-submit:not(:disabled):active {
