	...
	const BASE_BLOCKS = [
		{
			id: 'bio',
			type: 'text',
			label: 'Biografía / Texto Libre',
			content: 'Este es un bloque de texto que puedes personalizar en tu perfil.',
			enabled: true
		},
		{
			id: 'social',
				...
				.then((res) => {
					if (res?.user?.customization) {
						...
						if (c.blocks_layout) {
							try {
								let savedBlocks =
									typeof c.blocks_layout === 'string'
										? JSON.parse(c.blocks_layout)
										: c.blocks_layout;
...
<div
	class="immersive-canvas profile-custom-wrapper"
	style="
  {bgColor ? `background-color: ${bgColor};` : ''}
  {bgImageUrl
		? `background-image: url('${bgImageUrl}'); background-size: cover; background-position: center; background-attachment: fixed;`
		: ''}
...
>
						...
						<div class="flex gap-4 items-end">
							...
							<div class="pb-2">
								<h1 class="text-3xl font-bold" style="color: var(--text-primary);">
									{authStore.user?.display_name || 'Tu Nombre'}
								</h1>
					...
					{#if block.enabled}
						<div class="glass-card mockup-card p-6" transition:fade>
							{#if block.type === 'text'}
								...
								<p
									style="font-size: 1.05rem; line-height: 1.6; color: var(--text-secondary); white-space: pre-wrap;"
								>
									{block.content}
								</p>
							{:else if block.type === 'links'}
								...
								<div class="flex gap-2 flex-wrap">
									{#each block.links || [] as link}
										<span class="aero-badge-virtual px-3 py-1 cursor-pointer"
											>{link.title || 'Enlace'}</span
										>
									{/each}
								</div>
	...
	{#if panelOpen}
		<aside
			class="floating-properties-panel glass-panel shadow-neon"
		...
		>
			...
			<div class="panel-top-actions glass-panel">
				...
				<h2
					class="panel-title text-[1.1rem] font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-blue-600"
				>
					Re-Designer
				</h2>
				<a
			...
			<div class="panel-tabs">
				<button class:active={activeTab === 'style'} onclick={() => (activeTab = 'style')}
					>Estética</button
				>
			...
			<div class="panel-content">
				{#if message.text}
					<div
						class="alert {message.type === 'error'
							? 'alert-error'
							: 'alert-success'} mb-4 text-sm font-semibold py-3 text-center"
					>
						{message.text}
					</div>
				{/if}
				...
				{#if activeTab === 'style'}
					...
					<div class="props-section mt-6">
						...
						<div class="prop-col">
							<label for="glass-blur" class="flex justify-between"
								><span>Intensidad de Blur</span>
								<span class="font-bold text-primary">{glassBlur}px</span></label
							>
							<input
								id="glass-blur"
								name="glass-blur"
							...
							/>
							<span class="text-[10px] text-muted italic mt-1"
								>* Solo es visible si hay una imagen de fondo u otros elementos detrás del panel.</span
							>
						</div>
						<div class="prop-col mt-3">
							<span class="flex justify-between text-muted text-xs italic"
								>La Opacidad del panel actualmente hereda del tema de usuario.</span
							>
						</div>
					</div>
					<div class="props-section mt-6">
						...
						<div class="prop-col">
							<div
								class="relative w-full border-2 border-dashed border-blue-500/30 rounded-xl bg-blue-500/5 hover:bg-blue-500/10 transition-colors cursor-pointer group flex flex-col items-center justify-center py-6"
							>
								<input
									id="font-upload"
									name="font-upload"
								...
								/>
								<span
									class="material-icons-round text-3xl text-blue-500/70 group-hover:text-blue-500 mb-2 transition-colors"
									>font_download</span
								>
								<span
									class="text-sm font-bold text-main group-hover:text-blue-500 transition-colors"
									>Sube o suelta una fuente aquí</span
								>
							...
							{#if customFontUrl}
								<div class="mt-3 flex gap-2 items-center">
									...
									<input
										...
										class="aero-input-sm flex-1"
										placeholder="Nombre de tu fuente"
									/>
								</div>
							{/if}
						</div>
				...
				{:else if activeTab === 'blocks'}
					...
					<div class="props-section">
						...
						<p class="text-xs text-muted mb-4 leading-tight">
							Activa o desactiva los bloques de tu perfil. Despliega para editar su contenido.
						</p>
						...
						<div class="blocks-list">
							{#each blocksLayout as block, i}
								<div
									class="block-editor-card glass-card {block.enabled
										? 'is-active'
										: 'is-inactive'} flex flex-col"
								>
									<div class="flex items-center justify-between w-full">
										<div class="block-info flex-1">
											<span class="block-icon material-icons-round"
												>{block.type === 'text'
													? 'notes'
													: block.type === 'links'
														? 'link'
										...
										<div class="order-arrows flex flex-col">
											...
											<button
												class="arrow-btn"
												disabled={i === blocksLayout.length - 1}
									...
									{#if block.enabled}
										<div
											class="w-full mt-3 pt-3 border-t"
											style="border-color: rgba(255,255,255,0.1);"
										>
											{#if block.type === 'text'}
												<label for="block-content-{i}" class="text-xs text-muted mb-1 block"
												...
												<textarea
													...
													class="aero-textarea w-full text-sm p-2 bg-black/10 border-white/5 rounded-md"
													rows="2"
											...
											{:else if block.type === 'links'}
												...
												<div class="flex flex-col gap-2">
													{#each block.links || [] as link, li}
														<div class="flex gap-1 items-center">
															...
															<input
																...
																placeholder="Título"
																class="aero-input-sm w-24 bg-black/10"
															/>
															...
															<input
																...
																bind:value={link.url}
																placeholder="https://"
																class="aero-input-sm flex-1 bg-black/10"
															/>
															<button
																class="btn-icon bg-red-500/10 text-red-400 hover:bg-red-500/20"
													...
													{/each}
													<button
														class="btn-aero-secondary text-xs py-1 mt-1 border-dashed"
														onclick={() => addLink(block)}>+ Añadir Enlace</button
													>
												</div>
											{:else}
												<p class="text-xs text-muted italic text-center">
													Este bloque se llena automáticamente con tu contenido real.
												</p>
											{/if}
										</div>
									{/if}
			...
			<div class="panel-footer glass-panel">
				<button
					class="btn-aero-primary w-full"
					onclick={saveDesign}
					disabled={saving}
					style="padding: 12px; font-size: 0.95rem; font-weight: 700;"
				>
					{#if saving}
	...
	{#if !panelOpen}
		<button
			class="btn-toggle-panel glass-card shadow-neon"
		...
		<div class="advanced-css-content glass-panel shadow-neon">
			...
			>
				...
				<p class="text-muted mb-2">
					Usa esto solo si sabes lo que haces. Para evitar romper la estructura global de la red
					social, tus reglas deben estar precedidas por <code
						class="bg-black/10 px-1 rounded font-mono text-primary">.profile-custom-wrapper</code
					>.
				</p>
			...
			<div class="flex justify-end mt-4">
				<button class="btn-aero-primary" onclick={() => (showAdvancedCSS = false)}
					>Confirmar Inyección</button
				>
			</div>
		</div>
	</div>
