...
<div class="max-w-4xl mx-auto w-full space-y-6 pt-4 pb-20">
	<div class="glass-panel p-6 sm:p-8 relative overflow-hidden">
		...
		<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
			<div>
				<h1 class="text-3xl font-bold tracking-tight text-white/90 drop-shadow-md">
					Registro de Actividad
				</h1>
				<p class="text-white/50 text-sm mt-1">
					Todo lo que haces, visualizas e interactúas en V-SOCIAL queda registrado aquí.
				</p>
			</div>
		</div>
		...
		<div class="space-y-4">
			...
			{:else}
				{#each logs as log (log.id)}
					<div
						class="group relative flex gap-4 p-4 rounded-xl hover:bg-white/5 border border-transparent hover:border-white/5 transition-colors duration-200"
						transition:slide
					>
						<!-- Conector vertical (Timeline) -->
						<div
							class="absolute left-9 top-14 bottom-[-1rem] w-[1px] bg-white/10 group-last:hidden"
						></div>
						...
						<div class="flex-1 min-w-0 pt-1">
							<div class="flex justify-between items-baseline gap-2">
								...
								<span class="text-xs text-white/40 whitespace-nowrap shrink-0"
							...
							{#if log.entityPreview}
								<div
									class="mt-2 p-3 rounded-lg bg-black/20 border border-white/5 text-sm text-white/60 line-clamp-2"
								>
									{#if ['delete', 'update'].includes(log.action_type)}
										<span class="text-xs font-mono text-yellow-400/70 mb-1 block"
