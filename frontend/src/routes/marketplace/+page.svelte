<script>
  ...
  async function loadMarketplaceListings() {
    ...
    try {
      ...
      listings = [
        { id: 1, title: 'Modelo Live2D - Neko Maid', description: 'Modelo completo listo para VTube Studio con 4 expresiones y física en cabello.', price: 120, seller_username: 'neko_rigs', seller_display_name: 'Neko Rigs', category_id: 2, category_name: 'Modelos Live2D', media_url: 'https://images.unsplash.com/photo-1607604276583-eef5d076aa5f?w=600', ratings_avg: 4.8, created_at: new Date().toISOString() },
        { id: 2, title: 'Pack de Overlays Cyberpunk', description: 'Overlays animados para OBS con transiciones, alertas y escenas de inicio/fin.', price: 25, seller_username: 'cyber_draws', seller_display_name: 'Cyber Designs', category_id: 4, category_name: 'Efectos Stream', media_url: 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=600', ratings_avg: 4.5, created_at: new Date().toISOString() },
        { id: 3, title: 'Rigging Avanzado Custom', description: 'Servicio completo de rigging de busto o cuerpo entero. Plazo 2 semanas.', price: 350, seller_username: 'sora_chan', seller_display_name: 'Sora VTuber', category_id: 3, category_name: 'Rigging Live2D', media_url: 'https://images.unsplash.com/photo-1634017839464-5c339ebe3cb4?w=600', ratings_avg: 5.0, created_at: new Date().toISOString() }
      ];
    } finally {
      loading = false;
    }
  ...
  async function openItemDetails(item) {
    ...
    try {
      ...
      localReviews = [
        { id: 1, username: 'sakura_art', display_name: 'Sakura VT', rating: 5, comment: 'Excelente modelo, muy limpio el rig y las texturas.' },
        { id: 2, username: 'neon_fan', display_name: 'Neon Fan', rating: 4, comment: 'Buen overlay, fácil de configurar en OBS. Recomiendo.' }
      ];
    } catch (err) {
      selectedItem = item;
...
<svelte:head>
  <title>Marketplace  EVSocial</title>
</svelte:head>
...
<div class="marketplace-container">
  ...
  <div class="glass-card market-header">
    ...
    <div class="header-content">
      ...
      <p class="header-subtitle">Encuentra modelos Live2D, servicios de rigging, música y overlays exclusivos.</p>
    </div>
              ...
              <div class="item-details">
                <div>
                  <div class="item-header-meta">
                    <span class="aero-badge" style="font-size: 0.65rem;">{item.category_name || 'General'}</span>
                ...
                <div class="item-footer">
                  <span class="seller-tag">Por <span style="color: var(--text-main); font-weight: 700;">@{item.seller_username || item.username}</span></span>
                  <button
...
<!-- svelte-ignore a11y_no_static_element_interactions -->
  <div class="modal-backdrop" onclick={(e) => { if(e.target === e.currentTarget) { showCreateModal = false; createError = ''; } }} transition:fade={{ duration: 150 }}>
    <div class="glass-card aero-modal" style="max-width: 440px; width: 100%; padding: 24px;" transition:scale={{ duration: 250, start: 0.95, easing: backOut }}>
      ...
      <form onsubmit={handleCreateListing} class="modal-form">
        ...
        <div class="form-group">
          ...
          <div class="file-uploader-row">
            <button type="button" class="btn-aero-secondary btn-sm" onclick={handleSelectCreateFile}>
        ...
        >
          {#if creating}
            <span class="loading loading-spinner text-primary loading-xs" style="margin-right: 8px;"></span>
          {/if}
          Publicar listado
        </button>
      </form>
...
<!-- svelte-ignore a11y_click_events_have_key_events -->
<!-- svelte-ignore a11y_no_static_element_interactions -->
  <div class="modal-backdrop" onclick={(e) => { if(e.target === e.currentTarget) selectedItem = null; }} transition:fade={{ duration: 150 }}>
    <div class="glass-card detail-modal" transition:scale={{ duration: 250, start: 0.95, easing: backOut }}>
      ...
      <div class="detail-info-pane">
          ...
          {:else}
            ...
            <form onsubmit={handleSendOffer} class="offer-form">
              <div class="offer-input-row">
                ...
                >
                  {#if offering}
                    <span class="loading loading-spinner text-primary loading-xs" style="margin-right: 4px;"></span>
                  {/if}
                  Enviar
                </button>
              </div>
        ...
        <div class="detail-section" style="flex: 1; display: flex; flex-direction: column; min-height: 180px;">
            ...
            <form onsubmit={handlePostReview} class="review-form">
              <div class="review-input-row" style="align-items: center;">
                ...
                <input
                  ...
                  style="font-size: 0.75rem; padding-top: 6px; padding-bottom: 6px;"
                />
                <button type="submit" disabled={submittingReview} class="btn-aero-primary btn-sm" style="padding: 6px 12px;">
                  Publicar
                </button>
              </div>
            </form>
        ...
        <div class="seller-card">
          <div class="seller-avatar">
            <span>{(selectedItem.seller_username || selectedItem.username || '?')[0].toUpperCase()}</span>
          </div>
          <div>
            <p class="seller-name-title">Vendido por @{selectedItem.seller_username || selectedItem.username}</p>
