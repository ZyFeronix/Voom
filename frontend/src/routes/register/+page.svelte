...
<svelte:head>
  <title>Crear cuenta — VSocial</title>
  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin="" />
  <link href="https://fonts.googleapis.com/css2?family=Material+Icons+Round" rel="stylesheet" />
</svelte:head>

<div class="register-container min-h-screen w-full flex relative overflow-hidden" in:fade={{ duration: 600 }}>
    ...
    <div class="glass-panel p-8 max-w-md text-center flex flex-col items-center gap-6 animate-float">
      ...
      <div>
        ...
        <p class="text-sm text-muted leading-relaxed">
          Crea tu identidad virtual en la primera red social diseñada específicamente para VTubers, ilustradores digitales y sus comunidades.
        </p>
      </div>

      <div class="w-full flex items-center justify-between text-xs text-muted border-t pt-4" style="border-color: var(--border-color);">
        <span>&copy; 2026 VSocial Inc.</span>
        <div class="flex gap-4">
          <a href="/" class="text-link">Términos</a>
  ...
  <main class="w-full lg:w-[50%] flex items-center justify-center p-6 sm:p-12 z-10 relative" in:fly={{ y: 20, duration: 800, delay: 300 }}>
    ...
    <div class="w-full max-w-md my-4 {mounted ? 'visible' : 'hidden-state'} transition-all duration-500">
      
      <!-- Progress Indicator -->
      <div class="mb-4">
        <div class="flex justify-between items-center text-[10px] font-bold text-muted mb-2 uppercase tracking-wide">
          <span>PASO {step} DE 3</span>
          <span class="text-gradient">
            {step === 1 ? 'DATOS DE ACCESO' : step === 2 ? 'TUS INTERESES' : 'TIPO DE PERFIL'}
          </span>
        </div>
        <div class="w-full rounded-full overflow-hidden" style="height: 4px; background: var(--border-glass);">
          <div 
            class="h-full transition-all duration-300"
            style="width: {step * 33.33}%; background: var(--grad-primary); box-shadow: 0 0 10px rgba(0, 119, 255, 0.5);"
          ></div>
        </div>
      </div>
      ...
      <div class="glass-panel p-8">
        ...
        {#if step === 1}
          <div class="animate-fade-in flex flex-col gap-4">
            <div>
              ...
              <p class="text-xs text-muted">Introduce tus datos básicos para registrarte en el metaverso.</p>
            </div>

            <form onsubmit={(e) => { e.preventDefault(); nextStep(); }} class="flex flex-col gap-4">
              <div class="flex flex-col gap-1.5">
                <label for="username" class="text-xs font-bold text-muted">Usuario</label>
                <div class="relative" style="position: relative;">
                  <span class="text-muted font-bold text-sm" style="position: absolute; left: 14px; top: 50%; transform: translateY(-50%); z-index: 2; pointer-events: none;">@</span>
                  <input
                    id="username"
                    type="text"
                    required
                ...
                {#if username && username.length < 3}
                  <p class="text-[10px] text-red-500 font-semibold">Debe tener al menos 3 caracteres.</p>
                {/if}
              </div>
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div class="relative" style="position: relative;">
                  <span class="material-icons-round text-muted text-[18px]" style="position: absolute; left: 14px; top: 50%; transform: translateY(-50%); z-index: 2; pointer-events: none;">email</span>
                  <input
                    id="email"
                    type="email"
                    required
                ...
                {#if email && !emailValid}
                  <p class="text-[10px] text-red-500 font-semibold">Introduce un correo electrónico válido.</p>
                {/if}
              </div>
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div class="relative" style="position: relative;">
                  <span class="material-icons-round text-muted text-[18px]" style="position: absolute; left: 14px; top: 50%; transform: translateY(-50%); z-index: 2; pointer-events: none;">lock</span>
                  <input
                    id="password"
                ...
                {#if password && password.length < 8}
                  <p class="text-[10px] text-red-500 font-semibold">Debe tener al menos 8 caracteres.</p>
                {/if}
              </div>

              <div class="flex flex-col gap-1.5">
                <label for="confirmPassword" class="text-xs font-bold text-muted">Confirmar Contraseña</label>
                <input
                  id="confirmPassword"
                  type="password"
                  required
                ...
                {#if confirmPassword && !passwordsMatch}
                  <p class="text-[10px] text-red-500 font-semibold">Las contraseñas no coinciden.</p>
                {/if}
              </div>
        ...
        {:else if step === 2}
          <div class="animate-fade-in flex flex-col gap-4">
            <div>
              ...
              <p class="text-xs text-muted">Elige exactamente 3 categorías para personalizar tu feed inicial.</p>
            </div>

            <div class="grid grid-cols-2 gap-2 max-h-[280px] overflow-y-auto pr-1 scrollbar-custom">
              {#each categories as cat}
                ...
                >
                  <span class="material-icons-round text-lg {selectedInterests.includes(cat.id) ? 'text-blue-500' : 'text-muted'}">
                    {cat.icon}
                  </span>
            ...
            {#if selectedInterests.length < 3}
              <p class="text-[10px] text-center text-muted font-bold">Selecciona {3 - selectedInterests.length} más para continuar</p>
            {/if}
          </div>
        ...
        {:else if step === 3}
          <div class="animate-fade-in flex flex-col gap-4">
            <div>
              <h2 class="text-xl font-black text-main title-display mb-1">¿Cómo usarás VSocial?</h2>
            ...
            <div class="flex flex-col gap-3">
              ...
              <button
                ...
                class="profile-option {profileType === 'viewer' ? 'selected viewer-selected' : ''}"
              >
                ...
                <div class="flex-1">
                  ...
                  <p class="text-[10px] text-muted mt-0.5">Seguir creadores, ver streams, comprar arte digital y participar del chat.</p>
                </div>
                {#if profileType === 'viewer'}
                  <div class="check-mark viewer-check">
                    <span class="material-icons-round text-white text-[10px] font-bold">check</span>
                  </div>
                {/if}
              </button>
              ...
              <button
                ...
                class="profile-option {profileType === 'creator' ? 'selected creator-selected' : ''}"
              >
                ...
                <div class="flex-1">
                  ...
                  <p class="text-[10px] text-muted mt-0.5">Soy VTuber, artista o creador. Publicar directos, asset packs y conectar con mi público.</p>
                </div>
...
<style>
  ...
  .viewer-icon { background: linear-gradient(135deg, var(--color-aero-cyan), var(--color-aero-blue)); }
  .creator-icon { background: linear-gradient(135deg, var(--color-aero-green), var(--color-aero-cyan)); }
  ...
  .viewer-check { background: var(--color-aero-blue); }
  .creator-check { background: var(--color-aero-green); }
  ...
  /* =========================================================
     ...
     ========================================================= */
  :global(:root) {
    --register-bg: radial-gradient(circle at 10% 25%, rgba(0, 229, 255, 0.2), transparent 45%),
