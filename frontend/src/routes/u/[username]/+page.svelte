<script>
  ...
  let editForm = $state({ bio: '', location: '', website: '' });
  let posts = $state([]);
  ...
  let currentLevelBaseXp = $derived(currentLevel <= 1 ? 0 : Math.pow(currentLevel - 1, 2) * XP_PER_LEVEL_FACTOR);
  ...
  let hasCustomBg = $derived(!!(user?.customization?.bg_color || user?.customization?.bg_image_url));
  let isLightBg = $derived(
    user?.customization?.bg_image_url ? false : 
    (user?.customization?.bg_color ? getLuminance(user.customization.bg_color) > 150 : false)
  );
</script>
...
<div 
  ...
  style:--glass-blur={user?.customization?.glass_blur ? `blur(${user.customization.glass_blur}px) saturate(1.2)` : undefined}
  style:--font-sans={user?.customization?.font_family ? `'${user.customization.font_family}', sans-serif` : undefined}
  style:--font-display={user?.customization?.font_family ? `'${user.customization.font_family}', sans-serif` : undefined}
  style="{user?.customization?.bg_image_url ? `background: url('${user.customization.bg_image_url}') center/cover fixed !important;` : ''}"
>
  ...
  {:else}
    ...
    <div class="profile-header-card glass-panel">
      ...
      <div class="profile-header">
        <div class="avatar-and-names">
          <div class="profile-avatar">
            {#if user.avatar_url}
              <img src={user.avatar_url} alt="Avatar de {user.display_name}" crossorigin="anonymous" />
            {:else}
          ...
          <div class="profile-credentials">
            <div class="display-name-row">
              ...
              <VerifiedBadge role={user.role} isVerified={user.is_verified == 1} size="24px" interactive={true} />
              {#if user.level != null}
        ...
        <div class="profile-actions-row">
          {#if isOwnProfile}
            <a href="/settings" class="btn-aero-glass" style="padding: 0 16px; font-size: 0.9rem; flex: 0 0 44px; min-height: 44px; display: inline-flex; align-items: center; justify-content: center;">
              <span class="material-icons-round text-[18px] mr-1">settings</span> Editar perfil
            </a>
          {:else}
            <button onclick={startDirectMessage} class="btn-aero-glass" style="padding: 0 16px; font-size: 0.9rem; flex: 0 0 44px; min-height: 44px; display: inline-flex; align-items: center; justify-content: center;">
      ...
      <div class="profile-stats-bar">
        <div class="stat-col"><span class="stat-num">{posts.length}</span> <span class="stat-label">Publicaciones</span></div>
        <a href="/u/{user.username}/following?tab=followers" class="stat-col interactive"><span class="stat-num">{followersCount}</span> <span class="stat-label">Seguidores</span></a>
        <a href="/u/{user.username}/following?tab=following" class="stat-col interactive"><span class="stat-num">{user.following_count || 0}</span> <span class="stat-label">Siguiendo</span></a>
      </div>
        ...
        <div class="profile-meta-row">
          ...
          {#if user.website}
            <span class="meta-item">
              ...
              <a href={user.website} target="_blank" rel="noopener noreferrer" class="meta-link">{user.website}</a>
            </span>
          {/if}
          <span class="meta-item">
            <span class="material-icons-round" style="font-size: 0.9rem;">calendar_today</span>
            Se unió en {new Date(user.created_at || user.joined_at || Date.now()).toLocaleDateString('es', { month: 'long', year: 'numeric' })}
          </span>
        </div>
      </div>
      ...
      <div class="smooth-transition-wrapper profile-content-area">
                ...
                <div class="glass-card about-card" style="margin-top: 16px;">
                  ...
                  <div class="setup-details">
                    <div class="setup-row">
                      ...
                      <span class="setup-val" style="color: var(--secondary);">{user.vtuber_profile.character_name || user.display_name}</span>
                    </div>
                    {#if user.vtuber_profile.lore}
                      <div class="setup-row" style="margin-top: 12px;">
                        <span class="setup-label">Historia / Lore</span>
                        <p class="about-card-desc" style="margin-top: 2px;">{user.vtuber_profile.lore}</p>
                      </div>
                    {/if}
                  </div>
                </div>
            ...
            <div class="about-side-info">
              {#if user.is_virtual && user.vtuber_profile}
                <div class="glass-card about-card">
                  ...
                  <div class="setup-details" style="display: flex; flex-direction: column; gap: 12px;">
        ...
        {:else if activeTab === 'trash'}
          ...
          {:else}
            <div class="profile-posts-list">
              <div class="glass-panel p-4 mb-4 flex items-center gap-3 text-sm rounded-lg" style="background: rgba(46, 134, 232, 0.05); border-color: rgba(46, 134, 232, 0.1);">
                <span class="material-icons-round text-[20px] text-primary">info</span>
                <span class="text-muted">Los posts eliminados permanecerán aquí durante 30 días antes de ser eliminados permanentemente.</span>
              </div>
              
              {#if restoreError}
                <div class="glass-panel p-3 mb-4 flex items-center gap-2 text-sm rounded-lg" style="background: rgba(232, 74, 114, 0.1); border-color: rgba(232, 74, 114, 0.2); color: var(--aero-rose);">
              ...
              {#each deletedPosts as post (post.id)}
                <div class="glass-panel p-4 flex justify-between items-center mb-3">
                   <div class="min-w-0 flex-1 mr-4">
                     <p class="text-sm text-main truncate font-semibold mb-1">Post #{post.id}</p>
                     <p class="text-xs text-muted truncate">{post.body || (post.media?.length ? '[Multimedia]' : 'Sin contenido')}</p>
                   </div>
                       <button onclick={() => restorePost(post.id)} class="btn-aero-primary" style="padding: 6px 14px; font-size: 0.8rem;">Restaurar</button>
                    </div>
                  {/each}
                </div>
              {/if}
...
<style>
  ...
  .profile-follow-btn {
    ...
    box-shadow: 0 4px 12px rgba(14, 165, 233, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.3);
  ...
  .profile-follow-btn:hover {
    ...
    box-shadow: 0 6px 16px rgba(14, 165, 233, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.4);
