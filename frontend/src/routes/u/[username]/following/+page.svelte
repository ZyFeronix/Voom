<script>
  ...
  let currentList = $derived(
    activeTab === 'verified' ? verifiedFollowers :
...
<div class="network-container {user?.customization?.bg_color ? 'has-custom-bg' : ''}" style={user?.customization?.bg_color ? `background-color: ${user.customization.bg_color}` : ''}>
  ...
  <nav class="network-tabs glass-panel">
    <button 
      class="tab-btn" class:active={activeTab === 'verified'} 
      onclick={() => setTab('verified')}
    >
      Seguidores verificados
    </button>
    <button 
      class="tab-btn" class:active={activeTab === 'followers'} 
      onclick={() => setTab('followers')}
    >
      Seguidores
    </button>
    <button 
      class="tab-btn" class:active={activeTab === 'following'} 
        ...
        {#each currentList as u (u.id || u.username)}
          <a href="/u/{u.username}" class="user-card glass-panel interactive">
            <div class="user-avatar">
              {#if u.avatar_url}
                <img src={u.avatar_url} alt="Avatar de {u.display_name || u.username}" crossorigin="anonymous" />
              {:else}
                <div class="avatar-fallback">{(u.display_name || u.username || '?').charAt(0).toUpperCase()}</div>
              {/if}
            </div>
            <div class="user-info">
              <div class="user-name-row">
                ...
                {#if u.is_verified == 1}
                  <VerifiedBadge role={u.role || 'user'} isVerified={true} size="16px" interactive={false} />
                {/if}
                {#if u.level != null}
            ...
            {#if authStore.user?.username !== u.username}
              <button 
                class={u.is_following ? 'btn-aero-glass following-btn' : 'btn-aero-primary follow-btn'}
